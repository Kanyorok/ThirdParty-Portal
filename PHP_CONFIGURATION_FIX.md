...
Our DocumentService tests used **small text content** (< 1KB), so they never hit PHP upload limits. Real portal uploads with **multi-MB files** hit the 2MB PHP limit and failed.
...
> **"We found the root cause of bid upload failures. It's a PHP configuration issue, not code. Please update php.ini with the settings in PHP_CONFIGURATION_FIX.md and restart Apache. This will resolve all 'storage exception' errors."**
