...
import {
  IE11OrLess, Edge, FireFox, Safari, IOS, ChromeForAndroid,
} from './BrowserInfo.js';
...
let ghostRelativeParentInitialScroll = []; // (left, top)
...
const _detectDirection = function (el, options) {
  const elCSS = css(el);
  const elWidth = parseInt(elCSS.width)
...
const _checkOutsideTargetEl = function (evt) {
  if (dragEl) {
...
Sortable.prototype = /** @lends Sortable.prototype */ {
  ...
  _onTouchMove(/** TouchEvent */evt) {
    if (tapEvt) {
      ...
      const dx = ((touch.clientX - tapEvt.clientX)
						+ fallbackOffset.x) / (scaleX || 1)
						+ (relativeScrollOffset ? (relativeScrollOffset[0] - ghostRelativeParentInitialScroll[0]) : 0) / (scaleX || 1);
      const dy = ((touch.clientY - tapEvt.clientY)
      ...
      if (ghostEl) {
        ...
        const cssMatrix = `matrix(${ghostMatrix.a},${ghostMatrix.b},${ghostMatrix.c},${ghostMatrix.d},${ghostMatrix.e},${ghostMatrix.f})`;
  ...
  _onDragOver(/** Event */evt) {
    ...
    if (activeSortable && !options.disabled
      ...
      if (!elLastChild || _ghostIsLast(evt, vertical, this) && !elLastChild.animated) {
        ...
        const moveVector = onMove(rootEl, el, dragEl, dragRect, target, targetRect, evt, after);
