...
4. `ReturnContentFragment` middleware detects these requests and returns only the Blade `@section('content')` (and any stacked `scripts`).
...
## Adding new pages
Ensure your Blade view extends the base layout and defines a `@section('content')`. Sidebar links should include `data-ajax="1"`.
