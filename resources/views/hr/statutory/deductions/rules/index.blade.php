...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h2 class="mb-0">{{ $deduction->Name }} Rules</h2>
            <div class="text-muted">Code: {{ $deduction->Code }}</div>
        </div>
        <a class="btn btn-primary" href="{{ route('hr.statutory.deductions.rules.create', $deduction->Id) }}">+ New Rule</a>
    </div>
                    ...
                    <tbody>
                        @forelse($rules as $rule)
                            <tr>
                                ...
                                <td class="text-end">
                                    <a class="btn btn-sm btn-outline-primary" href="{{ route('hr.statutory.deductions.rules.edit', [$deduction->Id, $rule->Id]) }}">Edit</a>
                                    <form class="d-inline" action="{{ route('hr.statutory.deductions.rules.destroy', [$deduction->Id, $rule->Id]) }}" method="POST" onsubmit="return confirm('Deactivate this rule?');">
                                        @csrf
                                        @method('DELETE')
