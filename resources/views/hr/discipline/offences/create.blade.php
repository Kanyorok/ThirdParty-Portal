...
@section('title', 'New Offence')

@section('content')
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0">Create Offence</h2>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('hr.discipline.offences.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="RecommendedSanctionID" class="form-select">
                            ...
                            @foreach($sanctions as $sanction)
                                <option value="{{ $sanction->Id }}" @selected(old('RecommendedSanctionID') == $sanction->Id)>{{ $sanction->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-12">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="HearingRequired" value="1" @checked(old('HearingRequired'))>
                            <label class="form-check-label">Hearing Required</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="SummaryDismissalAllowed" value="1" @checked(old('SummaryDismissalAllowed'))>
                            <label class="form-check-label">Summary Dismissal Allowed</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="RequiresEvidence" value="1" @checked(old('RequiresEvidence'))>
                            <label class="form-check-label">Requires Evidence</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="RequiresApproval" value="1" @checked(old('RequiresApproval'))>
