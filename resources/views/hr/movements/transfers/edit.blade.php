...
<div class="container-fluid py-4">
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('hr.movements.transfers.update', $transfer->Id) }}">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select name="EmployeeID" id="EmployeeID" class="form-select">
                            ...
                            @foreach($employees as $emp)
                                <option value="{{ $emp->Id }}"
                                    ...
                                    @selected(old('EmployeeID', $transfer->EmployeeID) == $emp->Id)>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="FromBranchID" class="form-select">
                            ...
                            @foreach($branches as $branch)
                                <option value="{{ $branch->Id }}" @selected(old('FromBranchID', $transfer->FromBranchID) == $branch->Id)>{{ $branch->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="ToBranchID" class="form-select">
                            <option value="">Select</option>
                            @foreach($branches as $branch)
                                <option value="{{ $branch->Id }}" @selected(old('ToBranchID', $transfer->ToBranchID) == $branch->Id)>{{ $branch->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="FromDepartmentID" class="form-select">
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept->Id }}" @selected(old('FromDepartmentID', $transfer->FromDepartmentID) == $dept->Id)>{{ $dept->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="ToDepartmentID" class="form-select">
                            <option value="">Select</option>
                            @foreach($departments as $dept)
                                <option value="{{ $dept->Id }}" @selected(old('ToDepartmentID', $transfer->ToDepartmentID) == $dept->Id)>{{ $dept->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="FromRoleID" class="form-select">
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->Id }}" @selected(old('FromRoleID', $transfer->FromRoleID) == $role->Id)>{{ $role->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="ToRoleID" class="form-select">
                            <option value="">Select</option>
                            @foreach($roles as $role)
                                <option value="{{ $role->Id }}" @selected(old('ToRoleID', $transfer->ToRoleID) == $role->Id)>{{ $role->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Effective Date *</label>
                        <input type="date" name="EffectiveDate" class="form-control" value="{{ old('EffectiveDate', optional($transfer->EffectiveDate)->format('Y-m-d')) }}" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Status</label>
                        <select name="Status" class="form-select">
                            @foreach(['Pending','Approved','Rejected'] as $st)
                                <option value="{{ $st }}" @selected(old('Status', $transfer->Status) == $st)>{{ $st }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-12">
                        <label class="form-label">Reason</label>
                        <textarea name="Reason" class="form-control" rows="2">{{ old('Reason', $transfer->Reason) }}</textarea>
                    </div>
                </div>
