...
@section('title', 'Edit KPI Goals')

@section('content')
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0">Edit KPI Goals</h2>
        <a class="btn btn-outline-secondary" href="{{ route('hr.kpi.goals.show', $goal->Id) }}">Back</a>
    </div>
    ...
    <form method="POST" action="{{ route('hr.kpi.goals.update', $goal->Id) }}">
        @csrf
        @method('PUT')
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="EmployeeID" class="form-select" required>
                            ...
                            @foreach($employees as $emp)
                                <option value="{{ $emp->Id }}" data-grade="{{ $emp->GradeID }}" data-role="{{ $emp->RoleID }}" @selected(old('EmployeeID', $goal->EmployeeID)==$emp->Id)>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="PeriodID" class="form-select" required>
                            ...
                            @foreach($periods as $period)
                                <option value="{{ $period->Id }}" @selected(old('PeriodID', $goal->PeriodID)==$period->Id)>{{ $period->Name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2">
                        ...
                        <select name="PeriodYear" class="form-select" required>
                            ...
                            @foreach($yearOptions as $year)
                                <option value="{{ $year }}" @selected(old('PeriodYear', $goal->PeriodYear)==$year)>{{ $year }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Segment *</label>
                        <select name="PeriodSegment" class="form-select" data-selected="{{ old('PeriodSegment', $goal->PeriodSegment ?? 1) }}">
                            <option value="">Select Period First</option>
                        </select>
                    </div>
                    <div class="col-md-12">
                        <label class="form-label">Notes</label>
                        <textarea name="Notes" rows="2" class="form-control">{{ old('Notes', $goal->Notes) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card shadow-sm">
            ...
            <div class="card-body p-0">
                <table class="table mb-0" id="itemsTable">
                    ...
                    <tbody>
                        @foreach($goal->items as $index => $item)
                            <tr class="item-row">
                                <td>
                                    <select name="Items[{{ $index }}][PerspectiveID]" class="form-select kpi-perspective">
                                        <option value="">Select Perspective</option>
                                        @foreach($perspectives as $perspective)
                                            <option value="{{ $perspective->Id }}" @selected($item->kpiItem?->PerspectiveID == $perspective->Id)>{{ $perspective->Name }}</option>
                                        @endforeach
                                    </select>
                                    <div class="mt-1">
                                ...
                                <td>
                                    <select name="Items[{{ $index }}][KpiItemID]" class="form-select kpi-item" data-selected="{{ $item->KpiItemID }}" required>
                                        <option value="">Select Perspective First</option>
                                    </select>
                                </td>
                                <td><input type="number" step="0.01" name="Items[{{ $index }}][AnnualTarget]" class="form-control" value="{{ $item->AnnualTarget }}"></td>
                                <td><input type="number" step="0.01" name="Items[{{ $index }}][PeriodTarget]" class="form-control" value="{{ $item->PeriodTarget ?? $item->TargetValue }}"></td>
                                <td><input type="number" step="0.01" name="Items[{{ $index }}][Weight]" class="form-control kpi-weight" value="{{ $item->Weight }}" required></td>
...
<script>
    (function () {
        ...
        let index = table.querySelectorAll('.item-row').length;
        ...
        function populateKpisForRow(row) {
            ...
            const selectedKpi = kpiSelect.dataset.selected ? parseInt(kpiSelect.dataset.selected, 10) : null;
            kpiSelect.innerHTML = '';
            ...
            options.forEach(item => {
                ...
                if (selectedKpi && item.Id === selectedKpi) {
                    opt.selected = true;
                }
                kpiSelect.appendChild(opt);
            });
            kpiSelect.dataset.selected = '';
            updateWeightSummary();
        }
