...
<div class="container mt-4">
        ...
        <div class="mb-3">
            ...
            <select name="ReqId" id="ReqId" class="form-select" onchange="this.form.submit()">
                ...
                @foreach($pendingRequisitions as $requisitionOption)
                    <option value="{{ $requisitionOption->Id }}" {{ old('ReqId', request()->ReqId) == $requisitionOption->Id ? 'selected' : '' }}>
                        {{ $requisitionOption->ReqNo }} ({{ $requisitionOption->fromBranch?->Name ?? '?' }} → {{ $requisitionOption->toBranch?->Name ?? '?' }})
                    </option>
                @endforeach
            </select>
        </div>
    ...
    <div id="requisition-details" class="mt-4">
        ...
        <div class="row mb-4 bg-light p-3 border rounded">
            ...
            <div class="col-md-4"><strong>Date:</strong> {{ $requisition->CreatedOn ? $requisition->CreatedOn->format('Y-m-d') : 'N/A' }}</div>
