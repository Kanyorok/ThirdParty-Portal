                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="branch" name="Branch" required>
                        ...
                        @foreach($branches as $branch)
                            <option
                                value="{{ $branch->Id }}" {{ $adjustment->Branch == $branch->Id ? 'selected' : '' }}>
                                {{ $branch->Name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <table class="table table-bordered align-middle">
                    ...
                    @foreach ($adjustment->items as $index => $item)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>
                                <input type="text" class="form-control" value="{{ $item->item->ItemCode ?? '' }}"
                                       readonly>
                                <input type="hidden" name="items[{{ $index }}][Item]" value="{{ $item->Item }}">
                            </td>
                            <td>
                                <input type="text" class="form-control" value="{{ $item->item->ItemName ?? '' }}"
                                       readonly>
                            </td>
                            <td>
                                <input type="number" class="form-control current-qty"
                                       value="{{ $item->stockItem->CurrentQty ?? 0 }}" readonly>
                            </td>
                            <td>
