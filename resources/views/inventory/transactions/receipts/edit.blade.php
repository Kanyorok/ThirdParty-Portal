        ...
        <div class="card mb-3 shadow">
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="ReceivedBy" value="{{ old('ReceivedBy', $receipt->ReceivedBy) }}" required>
                </div>

                <div class="mb-3">
                    <label for="ReceivedDate" class="form-label">Received Date</label>
                    <input type="date" class="form-control" name="ReceivedDate" value="{{ old('ReceivedDate', $receipt->ReceivedDate) }}" required>
                </div>

                <div class="mb-3">
                    <label for="GeneralRemarks" class="form-label">General Remarks</label>
                    <textarea class="form-control" name="GeneralRemarks" rows="3">{{ old('GeneralRemarks', $receipt->GeneralRemarks) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <tbody>
                @foreach ($receipt->items as $index => $item)
                    ...
                    <tr>
                        <td>
                            <select name="items[{{ $index }}][item]" class="form-control" required>
                                @foreach ($receipt->transfer->items as $transferItem)
                                    <option value="{{ $transferItem->Item }}" {{ $item->Item == $transferItem->Item ? 'selected' : '' }}>
                        ...
                        <td>
                            <input type="number" class="form-control dispatched-qty" value="{{ $dispatchedQty }}" readonly>
                            <input type="hidden" name="items[{{ $index }}][dispatched_qty]" value="{{ $dispatchedQty }}">
                        </td>
                        <td>
                            <input type="number" name="items[{{ $index }}][received_qty]" class="form-control received-qty" value="{{ $item->ReceivedQty }}" required>
                        </td>
                        <td>
                        ...
                        <td>
                            <input type="number" name="items[{{ $index }}][damaged_qty]" class="form-control" value="{{ $item->DamagedQty }}">
                        </td>
                        <td>
