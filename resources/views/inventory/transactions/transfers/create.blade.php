    ...
    <form method="GET" action="{{ route('transactionstransfers.create') }}" class="mb-3">
        <div class="row">
            <div class="col-md-4">
                ...
                <select class="form-select" id="requisition_id" name="requisition_id" onchange="this.form.submit()"
                        required>
                    <option value="">Select Approved Requisition</option>
                    @foreach($approvedRequisitions as $req)
                        <option
                            value="{{ $req->Id }}" {{ (isset($requisition) && $requisition->Id == $req->Id) ? 'selected' : '' }}>
                            {{ $req->ReqNo }}
                        </option>
                    @endforeach
                </select>
        ...
        @if(isset($requisition))
            <div class="row mb-3">
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" class="form-control" id="fromBranch"
                           value="{{ $requisition->fromBranch->Name ?? '' }}" readonly>
                    <input type="hidden" name="FromBranch" value="{{ $requisition->FromBranch }}">
                </div>
                <div class="col-md-2">
                    <label for="toBranch" class="form-label">To Branch</label>
                    <input type="text" class="form-control" id="toBranch"
                           value="{{ $requisition->toBranch->Name ?? '' }}" readonly>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" id="transferredBy" name="TransferredBy"
                           value="{{ old('TransferredBy', Auth::user()->name ?? '') }}"> {{-- Pre-fill with current user's name --}}
                </div>
                    ...
                    <table class="table table-bordered align-middle" id="itemsTable">
                        ...
                        @foreach($requisition->items as $index => $item)
                            <tr>
                                ...
                                <td>
                                    <input type="number" class="form-control" name="items[{{ $index }}][approved_qty]"
                                           value="{{ $item->ApprovedQty }}" min="1" readonly>
                                </td>
                                <td>
                                    <input type="number" class="form-control" name="items[{{ $index }}][dispatched_qty]"
                                           value="{{ old('items.'.$index.'.dispatched_qty', $item->ApprovedQty) }}"
                                           min="0" max="{{ $item->ApprovedQty }}"
                                           required> {{-- Default to approved qty, cap at approved qty --}}
                                </td>
                                <td>
                        ...
                        @if(count($requisition->items) == 0)
                            <tr>
                                <td colspan="7" class="text-center">No items found for this requisition.
                                </td> {{-- Adjusted colspan --}}
                            </tr>
                        @endif
                        </tbody>
