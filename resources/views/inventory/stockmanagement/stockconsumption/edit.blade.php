        ...
        <form action="{{ route('stockconsumption.update', $stockConsumption->Id) }}" method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('BranchID') is-invalid @enderror" name="BranchID" id="BranchID"
                            required>
                        <option value="">Select Branch</option>
                        @foreach($branches as $branch)
                            <option
                                value="{{ $branch->Id }}" {{ old('BranchID', $stockConsumption->BranchID) == $branch->Id ? 'selected' : '' }}>
                                {{ $branch->Name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('ItemID') is-invalid @enderror" name="ItemID" id="ItemID"
                            required>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" class="form-control @error('Quantity') is-invalid @enderror" name="Quantity"
                           id="Quantity" value="{{ old('Quantity', $stockConsumption->Quantity) }}" required>
                    @error('Quantity')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select name="IssuedToType" id="IssuedToType"
                            class="form-select @error('IssuedToType') is-invalid @enderror" required>
                        <option value="">Select Option</option>
                        @foreach ($types as $type)
                            <option
                                value="{{ $type->ID }}" {{ old('IssuedToType', $stockConsumption->IssuedToType) == $type->ID ? 'selected' : '' }}>{{ $type->Description }}</option>
                        @endforeach
                    </select>
                    @error('IssuedToType')
                    <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="IssuedToID" id="IssuedToID"
                            class="form-select @error('IssuedToID') is-invalid @enderror" required>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select name="IssuedBy" id="IssuedBy" class="form-select @error('IssuedBy') is-invalid @enderror"
                            required>
                        <option value="">Select User</option>
                        @foreach($users as $user)
                            <option
                                value="{{ $user->Id }}" {{ old('IssuedBy', $stockConsumption->IssuedBy) == $user->Id ? 'selected' : '' }}>{{ $user->Name }}</option>
                        @endforeach
                    </select>
                    @error('IssuedBy')
                    <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="IssuedOn" id="IssuedOn"
                           ...
                           value="{{ old('IssuedOn', $stockConsumption->IssuedOn ? \Carbon\Carbon::parse($stockConsumption->IssuedOn)->format('Y-m-d') : now()->format('Y-m-d')) }}"
                           required>
                    @error('IssuedOn')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row mb-3">
                <div class="col-md-12">
                    ...
                    <textarea class="form-control @error('Remarks') is-invalid @enderror" name="Remarks" id="Remarks"
                              rows="3">{{ old('Remarks', $stockConsumption->Remarks) }}</textarea>
                    @error('Remarks')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
