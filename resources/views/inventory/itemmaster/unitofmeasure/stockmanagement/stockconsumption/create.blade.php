    ...
    <form action="{{ route('stockconsumption.store') }}" method="POST" id="consumptionForm">
                ...
                <select name="IssuedToID" id="IssuedToID" class="form-select @error('IssuedToID') is-invalid @enderror" required>
                    ...
                    @foreach($users as $user)
                        @php
                            ...
                            $displayName = ($employee ? $employee->FirstName . ' ' . $employee->LastName : $user->UserName) . ' (' . $user->UserName . ')';
                        @endphp
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                ...
                <div class="input-group">
                    @php
                        ...
                        $displayName = ($employee ? $employee->FirstName . ' ' . $employee->LastName : $currentUser->UserName) . ' (' . $currentUser->UserName . ')';
                    @endphp
            ...
            <div class="col-md-6">
                ...
                <small class="text-muted">Auto-set to current date ({{ \Carbon\Carbon::now()->format('F d, Y') }})</small>
                @error('IssuedOn')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                @enderror
            </div>
...
<script>
    $(document).ready(function () {
        ...
        let availableQty = 0;
        let issuedByUserId = currentUserId; // Set to current user
        ...
        function loadIssuedToOptions(type, oldIssuedToId) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // Filter out current user for EMPLOYEE type
                    data.forEach(item => {
        ...
        function validateUsers() {
            const issuedBy = issuedByUserId; // Always current user
