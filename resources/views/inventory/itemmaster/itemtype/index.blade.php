                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        <button type="button" class="btn btn-sm btn-info"
                                           onclick="showItemModal('{{ $itemtype->TypeName }}', '{{ $itemtype->StockTracked }}', '{{ $itemtype->RequiresTagging ? 1 : 0 }}', {{ $itemtype->Active ? 1 : 0 }})"
                                           title="View">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-warning"
                                           onclick="editItemModal('{{ route('itemtype.update', $itemtype->Id) }}', '{{ $itemtype->TypeName }}', '{{ $itemtype->StockTracked }}', '{{ $itemtype->RequiresTagging ? 1 : 0 }}', {{ $itemtype->Active ? 1 : 0 }})"
                                           title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-danger"
                                           onclick="confirmDelete('{{ $itemtype->Id }}')"
                                           title="Delete">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                        <form id="delete-form-{{ $itemtype->Id }}"
                                              action="{{ route('itemtype.destroy', $itemtype->Id) }}" method="POST"
                                              style="display:none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                ...
                <form id="editItemForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-check mb-3">
                            ...
                            <input class="form-check-input" type="checkbox" id="StockTracked" name="StockTracked"
                                   value="1">
                            <label class="form-check-label" for="StockTracked">Stock Tracked</label>
                        </div>
                        <div class="form-check mb-3">
                            <input type="hidden" name="RequiresTagging" value="0">
                            <input class="form-check-input" type="checkbox" id="editRequiresTagging"
                                   name="RequiresTagging" value="1">
