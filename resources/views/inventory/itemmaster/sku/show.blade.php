...
<div class="container bg-white shadow-sm rounded p-4">
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-4">
            <p><strong>{{ ucfirst($field) }}:</strong> {!! $item->$field ? '<i class="fas fa-check-circle text-success"></i>' : '<i class="fas fa-times-circle text-danger"></i>' !!}</p>
        </div>
        @endforeach
    </div>
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <p><strong>Status:</strong> {!! $item->Status ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-danger">Inactive</span>' !!}</p>
        </div>
    </div>
    ...
    <div class="d-flex gap-2 mt-3">
        ...
        <form action="{{ route('sku.destroy', $item->Id) }}" method="POST" onsubmit="return confirm('⚠️ Are you sure you want to delete this SKU?');">
            @csrf
            @method('DELETE')
