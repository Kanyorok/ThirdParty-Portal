    ...
    <form method="POST" action="{{ route('legal.ip_tracking.update', $tracking->ID) }}">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                ...
                <select name="TrackingType" class="form-control" required>
                    <option value="Renewal" {{ $tracking->TrackingType == 'Renewal' ? 'selected' : '' }}>Renewal</option>
                    <option value="Dispute" {{ $tracking->TrackingType == 'Dispute' ? 'selected' : '' }}>Dispute</option>
                </select>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                ...
                <input type="date" name="StartDate" class="form-control" value="{{ $tracking->StartDate }}" required>
            </div>
            ...
            <div class="col-md-4">
                ...
                <select name="Status" class="form-control">
                    ...
                    <option value="Resolved" {{ $tracking->Status == 'Resolved' ? 'selected' : '' }}>Resolved</option>
                </select>
            </div>
