        ...
        <form method="POST" action="{{ route('legal.obligations.update', $obligation->ID) }}">
            ...
            <div class="mb-3">
                ...
                <input type="date" name="DueDate" class="form-control"
                       value="{{ old('DueDate', \Carbon\Carbon::parse($obligation->DueDate)->format('Y-m-d')) }}"
                       required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="Status" class="form-select" required>
                    <option value="Pending" {{ old('Status', $obligation->Status) == 'Pending' ? 'selected' : '' }}>
                        Pending
                    </option>
                    <option value="Completed" {{ old('Status', $obligation->Status) == 'Completed' ? 'selected' : '' }}>
                        Completed
                    </option>
                </select>
            </div>
