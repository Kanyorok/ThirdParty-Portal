...
@section('content')
        ...
        <div class="card-header bg-light px-3 py-2 d-flex justify-content-between align-items-center">
            ...
            <div>
                ...
                <button class="btn btn-sm btn-info" data-bs-toggle="modal"
                        data-bs-target="#assignUserModal-{{ $obligation->Id }}">
    ...
    <form method="POST" action="{{ route('legal.obligations.assignUser', $obligation->Id) }}">
        @csrf
        @method('PATCH')
        <div class="modal fade" id="assignUserModal-{{ $obligation->Id }}" tabindex="-1"
             aria-labelledby="assignUserModalLabel-{{ $obligation->Id }}" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="UserId-{{ $obligation->Id }}" name="UserId" required>
                                <option value="" disabled {{ !$obligation->UserId ? 'selected' : '' }}>-- Choose User
                                    --
                                </option>
                                @foreach($users as $user)
                                    <option
                                        value="{{ $user->Id }}" {{ $obligation->UserId == $user->Id ? 'selected' : '' }}>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary"
                                onclick="if(this.form.checkValidity()){this.disabled=true; this.innerText='Assigning...'; this.form.submit();}">
                            Assign
                        </button>
                    </div>
