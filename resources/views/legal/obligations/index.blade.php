...
@section('content')
    <div class="card p-4 shadow rounded-4">
        <div class="card-header bg-light px-3 py-2 d-flex justify-content-between mb-1">
            ...
            <div>
                <a href="{{ route('legal.obligations.create') }}" class="btn btn-info"><i class="fas fa-plus me-1"></i>
                    New Obligation</a>
                {{-- <a href="{{ route('legal.obligations.calendar') }}" class="btn btn-outline-secondary">📆 Calendar View</a> --}}
            </div>
        </div>
        <div class="card-body">
            <p class="text-muted">Maintain and track all legal obligations to ensure compliance with regulatory,
                contractual, and organizational requirements.</p>
                    ...
                    @if($obligations->count())
                        @foreach($obligations as $obligation)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('legal.obligations.show', $obligation->Id) }}"
        ...
        @foreach ($obligations as $item)
            ...
            <form id="editObligationsForm" method="POST" action="{{route('legal.obligations.update',$item->Id)}}">
                @csrf
                @method('PATCH')
                <div class="modal fade" id="EditObligationsModal-{{$item->Id}}" tabindex="-1" aria-hidden="true"
                     aria-labelledby="EditObligationsModalLabel">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header bg-light">
                                <h5 class="modal-title text-info" id="EditObligationsModalLabel"><i
                                        class="fas fa-edit"></i> Obligation</h5>
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="Title" class="form-control"
                                               value="{{ old('ObligationTitle', $item->Title) }}" required>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="SourceType" class="form-select">
                                            <option
                                                value="{{ old('SourceType', $obligation->SourceType)}}">{{ old('SourceType', $item->SourceType)}}</option>
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" name="DueDate" class="form-control"
                                               value="{{ old('DueDate', \Carbon\Carbon::parse($item->DueDate)->format('Y-m-d')) }}">
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="Status" class="form-select" required>
                                            <option
                                                value="Pending" {{ old('Status', $item->Status) == 'Pending' ? 'selected' : '' }}>
                                                Pending
                                            </option>
                                            <option
                                                value="Completed" {{ old('Status', $item->Status) == 'Completed' ? 'selected' : '' }}>
                                                Completed
                                            </option>
                                            <option
                                                value="Overdue" {{ old('Status', $item->Status) == 'Overdue' ? 'selected' : '' }}>
                                                Overdue
                                            </option>
                                        </select>
                                    </div>
