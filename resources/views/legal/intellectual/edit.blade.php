    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('legal.intellectual.update', $record->Id) }}">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="date" name="RegistrationDate" class="form-control" 
                        value="{{ $record->RegistrationDate ? \Carbon\Carbon::parse($record->RegistrationDate)->format('Y-m-d') : '' }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Expiry Date</label>
                    <input type="date" name="ExpiryDate" class="form-control" 
                        value="{{ $record->ExpiryDate ? \Carbon\Carbon::parse($record->ExpiryDate)->format('Y-m-d') : '' }}" required>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="Remarks" class="form-control" rows="3" placeholder="Additional notes or details" required>{{ $record->Remarks }}</textarea>
            </div>
