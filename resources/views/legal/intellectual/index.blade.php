    ...
    <div class="card shadow p-4 rounded-4">
                    ...
                    @if($records->count())
                        @foreach ($records as $record)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('legal.intellectual.show', $record->Id) }}"
                                       class="btn btn-sm btn-info">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('legal.intellectual.edit', $record->Id) }}"
        ...
        @foreach($records as $record)
            ...
            <form method="POST" action="{{ route('legal.intellectual.raiseDispute', $record->Id) }}">
                @csrf
                @method('PATCH')
                <div class="modal fade" id="disputedModal-{{ $record->Id }}" tabindex="-1"
                     aria-labelledby="disputedModalLabel-{{ $record->Id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="DisputeReason-{{ $record->Id }}" class="form-label">Dispute
                                        Reason</label>
                                    <textarea
                                        class="form-control"
                                ...
                                <div class="mb-3">
                                    ...
                                    <label for="IsDisputed-{{ $record->Id }}" class="form-label">Mark as
                                        Disputed</label>
                                </div>
                            </div>
