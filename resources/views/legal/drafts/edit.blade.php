        ...
        <form action="{{ route('legal.drafts.update', $draft->ID) }}" method="POST">
            ...
            <div class="card shadow p-4 rounded-4 mb-4">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="DraftTitle" class="form-control" required value="{{ old('DraftTitle', $draft->DraftTitle) }}">
                </div>

                <div class="mb-3">
                    <label for="DocumentType" class="form-label">Document Type</label>
                    <input type="text" name="DocumentType" class="form-control" value="{{ old('DocumentType', $draft->DocumentType) }}">
                </div>

                <div class="mb-3">
                    <label for="Description" class="form-label">Description</label>
                    <textarea name="Description" class="form-control" rows="2">{{ old('Description', $draft->Description) }}</textarea>
                </div>

                <div class="mb-3">
                    <label for="Content" class="form-label">Draft Content</label>
                    <textarea id="editor" name="Content" class="form-control" rows="10">{!! old('Content', $draft->Content) !!}</textarea>
                </div>
