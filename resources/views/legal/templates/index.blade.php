                ...
                <form class="row g-3 align-items-end mb-3" method="GET" action="{{ route('legal.templates.index') }}">
                    <div class="col-md-5">
                        ...
                        <input type="text" name="q" value="{{ $q }}" class="form-control"
                               placeholder="Title, type, version, description">
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table align-middle text-center">
                        ...
                        @forelse($templates as $t)
                            <tr>
                                <td>
                                    <a href="{{ route('legal.templates.show', $t->Id) }}"
                                       class="fw-semibold text-decoration-none">
                                        {{ $t->Title }}
                                    </a>
                                    @if($t->Description)
                                        <div
                                            class="small text-muted">{{ \Illuminate\Support\Str::limit($t->Description, 80) }}</div>
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div
                                        class="small">{{ optional($t->CreatedOn)->format('Y-m-d H:i') ?? \Carbon\Carbon::parse($t->CreatedOn)->format('Y-m-d H:i') }}</div>
                                </td>
                                <td>
                                    @if($t->DocumentDMSID)
                                        <span class="badge text-bg-primary"
                                              title="Archived in DMS">#{{ $t->DocumentDMSID }}</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted py-4">No templates yet.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
