...
<div class="row">
    <!-- Main Form -->
    <div class="col-md-9">
        <form action="{{ route('legal.templates.update', $template->ID) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="card shadow p-4 rounded-4 mb-4">
                <h4 class="mb-4">✏️ Edit Template</h4>

                <div class="mb-3">
                    <label for="TemplateName" class="form-label">Template Name</label>
                    <input type="text" name="TemplateName" class="form-control" required value="{{ $template->TemplateName }}">
                </div>

                <div class="mb-3">
                    <label for="DocumentType" class="form-label">Document Type</label>
                    <input type="text" name="DocumentType" class="form-control" value="{{ $template->DocumentType }}">
                </div>

                <div class="mb-3">
                    <label for="Version" class="form-label">Version</label>
                    <input type="text" name="Version" class="form-control" value="{{ $template->Version }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="TemplateBody" id="editor" class="form-control" rows="10">{{ $template->TemplateBody }}</textarea>
                </div>
