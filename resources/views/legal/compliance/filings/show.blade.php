...
@section('content')
    <div class="card shadow rounded-4 p-4">
        ...
        @if($filing->FilePath)
            <p><strong>File:</strong> <a href="{{ Storage::url($filing->FilePath) }}"
                                         target="_blank">{{ $filing->FileName }}</a></p>
        @endif

        <hr>
        <h5>📂 Acknowledgments</h5>
        <form method="POST" action="{{ route('legal.compliance.filings.uploadAck',$filing->Id) }}"
              enctype="multipart/form-data">
            @csrf
            <div class="row mb-3">
                <div class="col-md-8"><input type="file" name="AckFile" class="form-control" required></div>
                <div class="col-md-4">
                    <button class="btn btn-primary">⬆️ Upload Ack</button>
                </div>
            </div>
        </form>
