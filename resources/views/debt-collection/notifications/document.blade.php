    ...
    <div id="MainContainer" class="form-container">
        ...
        <table style="border: 1px" id="activities">
            ...
            @foreach($notifications->sms as $notification)
                @php
                    $debt = \App\Models\BR\DebtProduct::where('AccountID', $notification->SourceID)->latest('processDate')->first();
                @endphp
                <tr>
                    <td>{{ \Illuminate\Support\Str::upper($notification->SMSId) }}.</td>
                    <td>{{ ($debt instanceof \App\Models\BR\DebtProduct)?$debt->ProductName:'? ?' }}</td>
                    <td>{{ ($debt instanceof \App\Models\BR\DebtProduct)?$debt->AccountID:'? ?' }}</td>
                    <td>{{ ($debt instanceof \App\Models\BR\DebtProduct)?$debt->ClientID:'? ?' }}</td>
                    <td>{{ ($debt instanceof \App\Models\BR\DebtProduct)?$debt->AccountName:'? ?' }}</td>
