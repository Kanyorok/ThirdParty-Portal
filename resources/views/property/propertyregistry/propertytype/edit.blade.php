...
@endif
    ...
    <form action="{{ route('propertytype.update', $type->Id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="PropertyTypeName" class="form-control" value="{{ old('PropertyTypeName', $type->PropertyTypeName) }}" required>
        </div>
            ...
            <label for="PropertyCategoryId" class="form-label">Category</label>
                <select name="PropertyCategoryId" class="form-select" required>
                    @foreach ($categories as $category)
                        <option value="{{ $category->Id }}" {{ $type->PropertyCategoryId == $category->Id ? 'selected' : '' }}>
                            {{ $category->Name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="mb-3">
            ...
            <textarea name="Description" class="form-control" rows="4">{{ old('Description', $type->Description) }}</textarea>
        </div>
