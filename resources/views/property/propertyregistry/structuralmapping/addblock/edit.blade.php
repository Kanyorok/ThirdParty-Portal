        ...
        <div class="mb-3">
            ...
            <select name="PropertyID" class="form-select" required>
                ...
                @foreach ($properties as $property)
                    <option
                        value="{{ $property->Id }}" {{ old('PropertyID', $block->PropertyID) == $property->Id ? 'selected' : '' }}>{{ $property->PropertyName }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
            <label for="BlockName" class="form-label">Block Name</label>
            <input type="text" name="BlockName" class="form-control" value="{{ old('BlockName', $block->BlockName) }}"
                   required>
        </div>
        <div class="mb-3">
            <label for="Description" class="form-label">Description </label>
            <textarea name="Description" class="form-control"
                      rows="4">{{ old('Description', $block->Description) }}</textarea>
        </div>
