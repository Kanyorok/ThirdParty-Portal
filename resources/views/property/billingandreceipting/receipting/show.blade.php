        ...
        <div class="card-body">
            ...
            <div class="row g-3 text-dark">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control bg-light text-dark"
                           value="{{ $receipt->InvoiceDate ? Carbon::parse($receipt->InvoiceDate)->format('d/m/Y') : '-' }}"
                           readonly>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control bg-light text-dark"
                           value="{{ $receipt->PaymentDate ? Carbon::parse($receipt->PaymentDate)->format('d/m/Y') : '-' }}"
                           readonly>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <textarea class="form-control bg-light text-dark" rows="2"
                              readonly>{{ $receipt->Remarks ?? '—' }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="card-footer d-flex justify-content-between align-items-center py-2 bg-light small text-dark">
                <div>
                    ...
                    on
                    <strong>{{ $receipt->CreatedOn ? Carbon::parse($receipt->CreatedOn)->format('d/m/Y') : '-' }}</strong>
                    | Modified by <strong>{{ $receipt->modifiedByUser->Name ?? '-' }}</strong>
                    on
                    <strong>{{ $receipt->ModifiedOn ? Carbon::parse($receipt->ModifiedOn)->format('d/m/Y') : '-' }}</strong>
                </div>
                <div>
