        ...
        <div class="card shadow-sm border-0 rounded-3">
            <div class="card-body">
                ...
                <div class="row g-3 text-dark">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control bg-light text-dark"
                               value="{{ $invoice->InvoiceDate ? Carbon::parse($invoice->InvoiceDate)->format('d/m/Y') : '-' }}"
                               readonly>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control bg-light text-dark" rows="3"
                                  readonly>{{ $invoice->InvoiceNotes ?? '—' }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card-footer d-flex justify-content-between align-items-center py-2 bg-light small text-dark">
                <div>
                    ...
                    on
                    <strong>{{ $invoice->CreatedOn ? Carbon::parse($invoice->CreatedOn)->format('d/m/Y') : '-' }}</strong>
                    | Modified by <strong>{{ $invoice->modifiedByUser->Name ?? '-' }}</strong>
                    on
                    <strong>{{ $invoice->ModifiedOn ? Carbon::parse($invoice->ModifiedOn)->format('d/m/Y') : '-' }}</strong>
                </div>
                <div>
