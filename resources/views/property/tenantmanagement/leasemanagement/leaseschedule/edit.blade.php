        ...
        @method('PUT')
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" id="property-display" class="form-control" readonly value="{{ old('PropertyName', $leaseschedules->lease->property->PropertyName ?? '') }}">
                        <input type="hidden" name="PropertyId" id="property-id" value="{{ old('PropertyId', $leaseschedules->lease->property->Id ?? '') }}">
                    </div>

                    <!-- Auto-filled Payment Frequency -->
                    <div class="col-md-6">
                        <label class="form-label">Payment Frequency</label>
                        <input type="text" id="frequency-display" class="form-control" readonly value="{{ old('FrequencyName', $leaseschedules->lease->code->Description ?? '') }}">
                        <input type="hidden" name="PaymentFrequency" id="frequency-id" value="{{ old('PaymentFrequency', $leaseschedules->lease->PaymentFrequency ?? '') }}">
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control" name="StartDate" value="{{ old('StartDate', $leaseschedules->StartDate ? \Carbon\Carbon::parse($leaseschedules->StartDate)->format('Y-m-d') : '') }}" >
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">End Date</label>
                            <input type="date" class="form-control" name="EndDate" value="{{ old('EndDate', $leaseschedules->EndDate ? \Carbon\Carbon::parse($leaseschedules->EndDate)->format('Y-m-d') : '') }}" >
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Base Rent per Period (KES)</label>
                            <input type="number" class="form-control"  name="BaseRent" value="{{ old('BaseRent', $leaseschedules->BaseRent) }}">
                        </div>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            <label class="form-label">Service Charge (KES)</label>
                            <input type="number" class="form-control"  name="ServiceCharge"value="{{ old('ServiceCharge', $leaseschedules->ServiceCharge) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Parking Fee (KES)</label>
                            <input type="number" class="form-control"  name="ParkingFee"value="{{ old('ParkingFee', $leaseschedules->ParkingFee) }}">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Other Charges (KES)</label>
                            <input type="number" class="form-control"  name="OtherCharges"value="{{ old('OtherCharges', $leaseschedules->OtherCharges) }}">
                        </div>
                    </div>
