  ...
  <div class="card shadow-sm mb-3">
    <div class="card-body d-flex align-items-center justify-content-between">
      <div>
        ...
        <div class="text-muted small">RFQ: <strong>{{ $rfq->RFQNumber ?? $rfqId }}</strong> · Evaluators: <strong>{{ $evaluatorCount }}</strong></div>
      </div>
      <form class="d-flex align-items-center gap-2" method="GET">
        ...
        <select name="rfq" class="form-select form-select-sm" onchange="this.form.submit()">
          @foreach($evaluatedRfqs as $opt)
            <option value="{{ $opt['Id'] }}" {{ (int)request('rfq', $rfqId) === (int)$opt['Id'] ? 'selected' : '' }}>{{ $opt['RFQNumber'] }}</option>
          @endforeach
        </select>
      </form>
    </div>
        ...
        <tbody>
          @forelse ($supplierSummaries as $i => $sup)
            <tr>
                    ...
                    <td>
                      <div class="d-flex flex-column text-center">
                        ...
                        <div class="progress" style="height: 6px;">
                          <div class="progress-bar bg-info" style="width: {{ min(100, ($score/10)*100) }}%;"></div>
                        </div>
                      </div>
                    </td>
                  @endforeach
              ...
              <td>{{ $sup['rank'] }}</td>
              <td><span class="badge {{ $sup['recommendation']['class'] }}">{{ $sup['recommendation']['status'] }}</span></td>
              <td>
          ...
          @empty
            <tr><td colspan="{{ 4 + count($sectionColumns) }}" class="text-center">No data</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
