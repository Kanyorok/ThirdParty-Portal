...
@section('content')
  <div class="container py-4">
    ...
    <ul class="nav nav-tabs" role="tablist">
      <li class="nav-item">
        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tab-suppliers" type="button" role="tab">Suppliers</button>
      </li>
      <li class="nav-item">
        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-criteria" type="button" role="tab">Criteria Averages</button>
      </li>
      <li class="nav-item">
        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-sections" type="button" role="tab">Sections</button>
      </li>
    </ul>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <form class="d-flex" method="GET">
            <select name="filter" class="form-select form-select-sm" onchange="this.form.submit()">
              <option value="all" {{ request('filter','all')==='all' ? 'selected' : '' }}>All Responses</option>
              <option value="evaluated" {{ request('filter')==='evaluated' ? 'selected' : '' }}>Only Evaluated</option>
            </select>
          </form>
        </div>
            ...
            <tbody>
              @forelse ($supplierSummaries as $i => $sup)
                <tr>
                  ...
                  <td>
                    <form action="{{ route('evaluations.award', ['rfq' => $rfqId, 'supplier' => $sup['supplier_id']]) }}" method="POST">
                      @csrf
                      <input type="hidden" name="Comments" value="Awarded via consolidated view">
                      <button class="btn btn-sm {{ ($award && $award->SupplierId == $sup['supplier_id']) ? 'btn-success' : 'btn-outline-primary' }}">
              ...
              @empty
                <tr><td colspan="{{ 4 + count($sections) }}" class="text-center">No data</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
