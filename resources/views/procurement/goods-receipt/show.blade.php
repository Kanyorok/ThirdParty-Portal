                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-3">
                                            ...
                                            <p class="mb-1"><strong>PO Number:</strong> {{ $grnSummary['order_no'] }}</p>
                                            <p class="mb-1"><strong>Received Date:</strong> {{ $grnSummary['received_date']->format('d M Y') }}</p>
                                        </div>
                                        <div class="col-md-3">
                                            <h6>Supplier Information</h6>
                                            <p class="mb-1"><strong>Name:</strong> {{ $grnSummary['supplier_name'] }}</p>
                                        </div>
                                        <div class="col-md-3">
                                            <h6>Summary</h6>
                                            <p class="mb-1"><strong>Total Lines:</strong> {{ $grnSummary['total_lines'] }}</p>
                                            <p class="mb-1"><strong>Total Value:</strong> KES {{ number_format($grnSummary['total_value'], 2) }}</p>
                                        </div>
                                        <div class="col-md-3">
                                            <h6>Status</h6>
                                            <p class="mb-1">
                                                <strong>Processing:</strong> 
                                                <span class="badge bg-{{ $grnSummary['processing_status'] == 'All Processed' ? 'success' : ($grnSummary['processing_status'] == 'Has Errors' ? 'danger' : 'warning') }}">
                                                    {{ $grnSummary['processing_status'] }}
                                                </span>
                                            </p>
                                            @if($grnSummary['can_post'])
                                                <p class="mb-1"><span class="badge bg-success">Ready for Processing</span></p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                ...
                                @foreach($grnLines as $line)
                                    <tr>
                                        <td>
                                            ...
                                            @endif
                                            @if($line->ExpiryDate)
                                                <br><small><strong>Expires:</strong> {{ $line->ExpiryDate->format('d M Y') }}</small>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td><strong>{{ number_format($line->ReceivedQTY, 2) }}</strong> {{ $line->item->uom->Name ?? '' }}</td>
                                        ...
                                        <td>
                                            ...
                                            @if($line->QualityRemarks)
                                                <br><small class="text-muted">{{ Str::limit($line->QualityRemarks, 30) }}</small>
                                            @endif
                                        </td>
                                        <td>
                                            ...
                                            @if($line->ProcessingErrors)
                                                <br><small class="text-danger">{{ Str::limit($line->ProcessingErrors, 30) }}</small>
                                            @endif
                                        </td>
                                        <td>
            ...
            <form id="qualityForm">
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea class="form-control" id="quality_remarks" rows="3" placeholder="Add remarks for quality check..."></textarea>
                    </div>
                </div>
