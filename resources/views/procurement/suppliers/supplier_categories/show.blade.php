    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white p-6">
            ...
            <div class="mb-4">
                ...
                <span
                    class="relative inline-block px-3 py-1 font-semibold text-{{ $supplierCategory->IsActive ? 'green' : 'red' }}-900 leading-tight">
                <span aria-hidden
                      class="absolute inset-0 bg-{{ $supplierCategory->IsActive ? 'green' : 'red' }}-200 opacity-50"></span>
                <span class="relative">{{ $supplierCategory->IsActive ? 'Active' : 'Inactive' }}</span>
            </span>
            </div>

            <div class="flex items-center justify-between">
                <a href="{{ route('procurement.suppliers.supplier-categories.edit', $supplierCategory) }}"
                   class="bg-indigo-500 hover:bg-indigo-700 text-white font-bold py-2 px-4 focus:outline-none">
                    Edit
                </a>
                <a href="{{ route('procurement.suppliers.supplier-categories.index') }}"
