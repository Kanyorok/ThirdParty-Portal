    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('prequalification.applications.show', $application->ApplicationID) }}"
               class="btn btn-light"
               hx-get="{{ route('prequalification.applications.show', $application->ApplicationID) }}"
               hx-target="#mainBodyContent" hx-swap="innerHTML" hx-push-url="true">
        ...
        <div class="card-body">
            <div class="row mb-3">
                <div class="col-md-8">
                    <p class="mb-1">
                        <strong>Supplier:</strong> {{ $application->supplier->thirdParty->ThirdPartyName ?? 'N/A' }}</p>
                    <p class="mb-1"><strong>Total
                            (Computed):</strong> {{ number_format($grandTotal ?? $result->TotalScore, 2) }}%</p>
                ...
                <div class="col-md-4 text-md-end">
                    <div class="h5 mb-0">Overall Score: <span class="text-primary">{{ number_format($grandTotal ?? $result->TotalScore, 2) }}%</span>
                    </div>
                </div>
            </div>
            <hr>
            ...
            <div class="mb-4">
                <h5 class="text-secondary mb-1">{{ $section['name'] }}
                    <small class="text-muted ms-2">(Section Weight: {{ number_format($section['sectionWeight'],2) }}% |
                        Achieved: {{ number_format($section['sectionScore'],2) }}%)</small>
                </h5>
                <div class="progress" style="height:6px;">
                    <div class="progress-bar bg-primary" role="progressbar"
                         style="width: {{ ($section['sectionWeight'] > 0 ? ($section['sectionScore'] / $section['sectionWeight']) * 100 : 0) }}%"
                         aria-valuenow="{{ $section['sectionScore'] }}" aria-valuemin="0"
                         aria-valuemax="{{ $section['sectionWeight'] }}"></div>
                </div>
                        ...
                        <tbody>
                            @foreach ($section['criteria'] as $criteria)
                                <tr>
                                    ...
                                    <td class="text-center">{{ number_format($criteria['criterionWeightShare'], 2) }}%
                                    </td>
                                    <td class="text-center fw-semibold">{{ number_format($criteria['weightedScore'], 2) }}
                                        %
                                    </td>
                                </tr>
                            @endforeach
