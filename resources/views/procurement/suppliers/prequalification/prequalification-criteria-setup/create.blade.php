    ...
    <div class="container-fluid py-4">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
                ...
                <a href="{{ route('prequalification.evaluation.index', $application->RoundId) }}"
            ...
            <form method="POST" action="{{ route('prequalification.evaluation.store', $application->ApplicationID) }}">
                        ...
                        <div class="accordion-item shadow-sm mb-3">
                            <h2 class="accordion-header" id="heading{{ $section->SectionID }}">
                                <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                    ...
                                    <h5 class="mb-0">{{ $section->masterSection->Name }} <small class="text-muted ms-2">({{ $section->Weight }}
                                            points)</small></h5>
                                </button>
                            </h2>
                                    ...
                                    @foreach($section->criteria as $criteria)
                                        ...
                                        <div class="card mb-3 shadow-sm border-left-primary">
                                            <div class="card-body">
                                                ...
                                                <div class="row g-3">
                                                    <div class="col-md-3">
                                                        <label for="evaluations[{{ $criteria->CriteriaID }}][Score]"
                                                               class="form-label">Score (Max: 10)</label>
                                                        <input type="number"
                                                               name="evaluations[{{ $criteria->CriteriaID }}][Score]"
                                                               id="evaluations[{{ $criteria->CriteriaID }}][Score]"
                                                               class="form-control"
                                                               value="{{ old("evaluations.{$criteria->CriteriaID}.Score", $evaluation->Score ?? '') }}"
                                                               min="0" max="10" required>
                                                    </div>
                                                    <div class="col-md-9">
                                                        <label for="evaluations[{{ $criteria->CriteriaID }}][Remarks]"
                                                               class="form-label">Remarks</label>
                                                        <textarea
                                                            name="evaluations[{{ $criteria->CriteriaID }}][Remarks]"
                                                            id="evaluations[{{ $criteria->CriteriaID }}][Remarks]"
                                                            class="form-control"
                                                            rows="2">{{ old("evaluations.{$criteria->CriteriaID}.Remarks", $evaluation->Remarks ?? '') }}</textarea>
                                                    </div>
                                                </div>
                                                <input type="hidden"
                                                       name="evaluations[{{ $criteria->CriteriaID }}][CriteriaID]"
                ...
                <div class="card-footer bg-white text-end">
                    <button type="submit"
                            class="btn btn-primary d-flex align-items-center justify-content-center ms-auto">
