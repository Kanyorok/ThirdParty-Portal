...
<form action="{{ $action }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <textarea name="Description" id="Description" class="form-control"
                  rows="3" {{ $readOnly ? 'readonly' : '' }}>{{ old('Description', $prequalificationRound->Description ?? '') }}</textarea>
    </div>
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-3">
            ...
            <select name="Status" id="Status"
                    class="form-select @error('Status') is-invalid @enderror" {{ $readOnly ? 'disabled' : '' }}>
    ...
    <div class="accordion" id="sectionsAccordion">
        @foreach($masterSections as $index => $section)
            ...
            <div class="accordion-item mb-2">
                ...
                <div id="collapse{{ $section->Id }}" class="accordion-collapse collapse {{ $isIncluded ? 'show' : '' }}"
                        ...
                        @if(!$readOnly)
                            ...
                            <div class="form-check mb-2">
                                ...
                                <input type="checkbox"
                                    ...
                                    {{ $isIncluded ? 'checked' : '' }}>
                                <label for="section_included_{{ $section->Id }}" class="mb-0">Include this
                                    section</label>
                            </div>
                                ...
                                @foreach($section->criteria as $c_index => $criteria)
                                    ...
                                    <div class="card mb-2">
                                        <div class="card-body py-2">
                                            <input type="hidden"
                                                   name="sections[{{ $index }}][criteria][{{ $c_index }}][criteria_id]"
                                                   value="{{ $criteria->Id }}">
                                            <div class="form-check">
                                                ...
                                                <label
                                                    for="criteria_included_{{ $criteria->Id }}">{{ $criteria->CriteriaName }}</label>
                                            </div>
                                            <div class="mt-2 criteria-weight-container"
                                                 style="{{ $isCriterionIncluded ? '' : 'display:none;' }}">
                            ...
                            @if($linkedCriteria->count())
                                <ul>
                                    @foreach($linkedCriteria as $criterion)
                                        <li>{{ $criterion->masterCriteria->CriteriaName }}
