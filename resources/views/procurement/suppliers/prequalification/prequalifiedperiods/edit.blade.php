    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('preqrounds.update', $period->Id) }}">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="MaxVendors" class="form-control" value="{{ old('MaxVendors', $period->MaxVendors) }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="Status" class="form-select">
                    @foreach(\App\Enums\Procurement\PrequalificationPeriodEnum::cases() as $status)
                        <option value="{{ $status->value }}" {{ old('Status', $period->Status->Label()) == $status->value ? 'selected' : '' }}>
                            {{ $status->Label() }}
                        </option>
                    @endforeach
                </select>
