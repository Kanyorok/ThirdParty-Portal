            ...
            <div class="card-body">
                ...
                @if(empty($consolidatedScores))
                    <div class="alert alert-warning">
                        <i class="fas fa-exclamation-triangle"></i> No responsive bids found or evaluations not
                        completed.
                    </div>
                @else
                    <form action="{{ route('procawards.store') }}" method="POST" id="awardForm">
                        ...
                        <table class="table table-bordered align-middle">
                            ...
                            @foreach($consolidatedScores as $index => $score)
                                <tr class="{{ $index === 0 ? 'table-success' : '' }}">
                                    ...
                                    <td class="text-center"><strong>{{ number_format($score['total_score'], 1) }}
                                            %</strong></td>
                                    <td class="text-center">
                                            <span
                                                class="badge {{ $score['is_responsive'] ? 'bg-success' : 'bg-danger' }}">
                        ...
                        @if(!$existingAward)
                            ...
                            <input type="hidden" name="technical_score" id="technical_score"
                                   value="{{ $consolidatedScores[0]['technical_score'] ?? 0 }}">
                            <input type="hidden" name="financial_score" id="financial_score"
                                   value="{{ $consolidatedScores[0]['financial_score'] ?? 0 }}">
                            <input type="hidden" name="total_score" id="total_score"
                                   value="{{ $consolidatedScores[0]['total_score'] ?? 0 }}">
                            ...
                            <div class="form-check mb-4">
                                <input class="form-check-input" type="checkbox" name="notify_unsuccessful" id="notify"
                                       value="1"
