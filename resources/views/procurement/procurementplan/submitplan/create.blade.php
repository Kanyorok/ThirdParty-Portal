...
<div class="container mt-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('Procurement-Plan-Submission.index') }}" class="btn btn-sm btn-outline-secondary">← Back to Plans</a>
  </div>
  ...
  <div class="row text-center mb-4">
    <div class="col-md-3"><div class="border p-3 rounded bg-white"><h6>Total Items</h6><h4>{{ $total }}</h4></div></div>
    <div class="col-md-3"><div class="border p-3 rounded bg-white"><h6>Budget Linked</h6><h4 class="text-warning">{{ $budgetLinked }} / {{ $total }}</h4></div></div>
    <div class="col-md-3"><div class="border p-3 rounded bg-white"><h6>Method Assigned</h6><h4 class="text-warning">{{ $methodAssigned }} / {{ $total }}</h4></div></div>
    <div class="col-md-3"><div class="border p-3 rounded bg-white"><h6>Scheduled</h6><h4 class="text-warning">{{ $scheduled }} / {{ $total }}</h4></div></div>
  </div>
  ...
  <div class="table-responsive mb-4">
    <table class="table table-bordered table-hover align-middle">
      ...
      <tbody>
        ...
        <tr>
          ...
          @if ($item->schedulePlan && $item->schedulePlan->periods->isNotEmpty())
            @foreach ($item->schedulePlan->periods as $period)
              {{ $period->SchedulePeriod }}: {{ $period->ScheduleQTY }}@if (!$loop->last), @endif
            @endforeach
          @else
        ...
        @empty
        <tr><td colspan="9" class="text-center">No line items found.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
