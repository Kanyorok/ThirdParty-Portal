        ...
        <div class="mb-3 col-md-6">
            ...
            <select id="CategoryID" class="form-select" disabled>
                @foreach ($categories as $category)
                    <option value="{{ $category->Id }}" {{ $lineItem->item->CategoryID == $category->Id ? 'selected' : '' }}>
                        {{ $category->Name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3 col-md-6">
            ...
            <select name="budget_line_id" id="budget_line_id" class="form-select" required>
                @foreach ($budgetLines as $budgetLine)
                    <option value="{{ $budgetLine->BudgetLineID }}" {{ $lineItem->BudgetLineID == $budgetLine->BudgetLineID ? 'selected' : '' }}>
