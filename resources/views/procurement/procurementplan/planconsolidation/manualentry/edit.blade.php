...
<div class="card p-4 shadow rounded-4">
  ...
  <form method="POST" action="{{ route('planmanualinput.update', $lineItem->LineItemID) }}">
    ...
    <div class="mb-3">
      ...
      <input type="number" name="quantity" id="quantity" class="form-control" value="{{ $lineItem->MergedQty }}" min="1" required>
    </div>

    <div class="mb-3">
      <label for="unit_of_measure" class="form-label">Unit of Measure</label>
      <input type="text" name="unit_of_measure" id="unit_of_measure" class="form-control" value="{{ $lineItem->UnitOfMeasure }}" required>
    </div>

    <div class="mb-3">
      <label for="estimated_cost" class="form-label">Estimated Cost</label>
      <input type="number" step="0.01" name="estimated_cost" id="estimated_cost" class="form-control" value="{{ $lineItem->EstimatedUnitCost }}" required>
    </div>

    <div class="mb-3">
      <label for="schedule_period" class="form-label">Planned Quarter</label>
      <input type="text" name="schedule_period" id="schedule_period" class="form-control" value="{{ $lineItem->SchedulePeriod }}" required>
    </div>

    <div class="mb-3">
      <label for="expected_delivery_date" class="form-label">Expected Delivery Date</label>
      <input type="date" name="expected_delivery_date" id="expected_delivery_date" class="form-control" value="{{ \Carbon\Carbon::parse($lineItem->ExpectedDeliveryDate)->format('Y-m-d') }}" required>
    </div>
    ...
    <a href="{{ route('procurement.procurementplan.planconsolidation.manualentry.index') }}" class="btn btn-secondary">Cancel</a>
  </form>
</div>
@endsection
