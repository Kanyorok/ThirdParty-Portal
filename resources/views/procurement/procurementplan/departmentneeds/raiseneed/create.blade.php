    ...
    <form action="{{ route('procurementdepartmentalplan.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <select name="ItemID" id="itemDropdown" class="form-select @error('ItemID') is-invalid @enderror" required>
            ...
            @foreach ($items as $item)
              <option value="{{ $item->Id }}" data-category="{{ $item->category->Name ?? '' }}"
                data-uom="{{ $item->uom->Name ?? 'N/A' }}" data-price="{{ $item->price->ActualPrice ?? '0.00' }}"
      ...
      <div class="row mb-3">
        <div class="col-md-6">
          ...
          <input type="number" name="RequestedQty" id="RequestedQty" value="{{ old('RequestedQty') }}"
            class="form-control @error('RequestedQty') is-invalid @enderror" placeholder="Enter quantity" required>
          @error('RequestedQty')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
      ...
      <div class="mb-3">
        ...
        <textarea name="Justification" id="Justification" rows="3"
          class="form-control @error('Justification') is-invalid @enderror" placeholder="Explain the need...">{{ old('Justification') }}</textarea>
        @error('Justification')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
      </div>
