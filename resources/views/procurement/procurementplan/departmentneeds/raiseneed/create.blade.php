  ...
  <form action="{{ route('procurementdepartmentalplan.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row mb-3">
      <div class="col-md-6">
        ...
        <input type="number" name="RequestedQty" id="RequestedQty" value="{{ old('RequestedQty') }}" class="form-control @error('RequestedQty') is-invalid @enderror" placeholder="Enter quantity" required>
        @error('RequestedQty')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
      </div>
    ...
    <div class="mb-3">
      ...
      <input type="number" step="0.01" name="EstimatedUnitCost" id="EstimatedUnitCost" value="{{ old('EstimatedUnitCost') }}" class="form-control @error('EstimatedUnitCost') is-invalid @enderror" placeholder="e.g. 100.50" required>
    ...
    <div class="mb-3">
      ...
      <textarea name="Justification" id="Justification" rows="3" class="form-control @error('Justification') is-invalid @enderror" placeholder="Explain the need...">{{ old('Justification') }}</textarea>
      @error('Justification')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
    ...
    <div class="mb-3">
      ...
      <input type="date" name="RequestedDate" id="RequestedDate" value="{{ old('RequestedDate') }}" class="form-control @error('RequestedDate') is-invalid @enderror" required>
      @error('RequestedDate')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
  </div>
