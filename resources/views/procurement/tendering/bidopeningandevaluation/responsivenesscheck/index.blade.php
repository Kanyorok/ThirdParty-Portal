    ...
    <div class="container mt-4">
        ...
        @if($selectedTender && $submissions->isNotEmpty())
            ...
            <div class="card-body">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h6 class="mb-1 text-primary">Responsiveness Check
                            for: {{ $selectedTender->TenderNo ?? 'Unknown Tender' }}</h6>
                        <p class="mb-0 text-muted">{{ $submissions->count() }} opened bids ready for responsiveness
                            evaluation</p>
                    </div>
            ...
            <div class="card shadow-sm">
                <div class="card-header">
                    <h5 class="mb-0"><i class="fas fa-clipboard-check"></i> Bid Submissions for Responsiveness Check
                    </h5>
                </div>
                    ...
                    <table class="table table-bordered table-striped align-middle mb-0">
                        ...
                        @foreach($submissions as $submission)
                            <tr id="submission-{{ $submission->Id }}">
                                ...
                                <td>
                                    @if($submission->isOpened())
                                        <strong>{{ $submission->Currency }} {{ number_format($submission->BidAmount, 2) }}</strong>
                                        <br>
                                        <small class="text-muted">
                                    ...
                                    @switch($submission->BidStatus)
                                        @case('responsive')
                                            ...
                                            @if($submission->ResponsivenessRemarks)
                                                <small
                                                    class="text-success">{{ Str::limit($submission->ResponsivenessRemarks, 50) }}</small>
                                            @endif
                                            @break
                                        @case('non-responsive')
                                            ...
                                            @if($submission->ResponsivenessRemarks)
                                                <small
                                                    class="text-danger">{{ Str::limit($submission->ResponsivenessRemarks, 50) }}</small>
                                            @endif
                                            @break
                                        @case('submitted')
                                            @if($submission->isOpened())
        ...
        @elseif($selectedTender && $submissions->isEmpty())
            <div class="alert alert-info">
                <i class="fas fa-info-circle"></i> No opened bids found for this tender. Bids must be opened in the
                opening ceremony before responsiveness check.
            </div>
        @else
            <div class="alert alert-info">
                <i class="fas fa-info-circle"></i> Please select a tender to view opened bids ready for responsiveness
                check.
            </div>
        @endif
    </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="submitted_timely" id="timing_yes"
                                           value="1" required>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="submitted_timely" id="timing_no"
                                           value="0" required>
                                    <label class="form-check-label text-danger" for="timing_no">
                                        <i class="fas fa-times"></i> Late Submission
                                    </label>
                                </div>
                                <textarea class="form-control mt-2" name="timely_remarks"
                                          placeholder="Remarks on submission timing..."></textarea>
                            </div>
                            <div class="col-md-6">
                                <h6 class="text-primary">Document Compliance</h6>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="has_mandatory_documents"
                                           id="docs_yes" value="1" required>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="has_mandatory_documents"
                                           id="docs_no" value="0" required>
                                    <label class="form-check-label text-danger" for="docs_no">
                                        <i class="fas fa-times"></i> Missing Documents
                                    </label>
                                </div>
                                <textarea class="form-control mt-2" name="document_remarks"
                                          placeholder="Remarks on document compliance..."></textarea>
                            </div>
                        </div>
                        <div class="row mt-3">
                            <div class="col-md-6">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="is_eligible" id="eligible_yes"
                                           value="1" required>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="is_eligible" id="eligible_no"
                                           value="0" required>
                                    <label class="form-check-label text-danger" for="eligible_no">
                                        <i class="fas fa-times"></i> Not Eligible
                                    </label>
                                </div>
                                <textarea class="form-control mt-2" name="eligibility_remarks"
                                          placeholder="Remarks on eligibility..."></textarea>
                            </div>
                            <div class="col-md-6">
                                <h6 class="text-primary">Overall Assessment</h6>
                                <textarea class="form-control" name="overall_remarks" rows="3"
                                          placeholder="Overall remarks on bid responsiveness..."></textarea>
                            </div>
                        </div>
                    </form>
                </div>
