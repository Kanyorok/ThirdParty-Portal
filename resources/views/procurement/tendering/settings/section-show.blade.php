            ...
            @foreach ($section->criteria as $criterion)
                <tr>
                    @if (isset($section) && isset($criterion))
                        <form
                            action="{{ route('prequalification.sections.criteria.update', [$section->Id, $criterion->Id]) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <td class="align-middle">
                                <select name="IsActive" class="form-select form-select-sm">
                                    <option
                                        value="1" {{ old('IsActive', $criterion->IsActive) == 1 ? 'selected' : '' }}>
                                        Active
                                    </option>
                                    <option
                                        value="0" {{ old('IsActive', $criterion->IsActive) == 0 ? 'selected' : '' }}>
                                        Inactive
                                    </option>
                                </select>
                            </td>
