          ...
          <tbody>
            @foreach ($criterias as $item)
              <tr>
                ...
                <td>
                  <a href="#" class="btn btn-sm btn-outline-primary" title="Edit" data-bs-toggle="modal"
                    data-bs-target="#editCriteriaModal" data-id="{{ $item->id }}"
                    data-name="{{ $item->CriteriaName }}" data-desc="{{ $item->Description }}">
                    <i class="fas fa-edit"></i>
                  </a>

                  <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal"
                    data-bs-target="#deleteCriteriaModal" data-id="{{ $item->id }}"
                    data-name="{{ $item->CriteriaName }}" title="Delete">
  ...
  <div class="modal fade" id="addSectionModal" tabindex="-1" aria-labelledby="addItemModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content rounded-3 shadow">
        ...
        <form action="{{ route('procurement.criterias.store') }}" method="POST">
          @csrf
          @method('POST')
          <input type="hidden" name="section_id" value="{{ $sectionID }}">
