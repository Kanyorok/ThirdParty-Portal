...
$(document).on('change', '#directPlanSelect', function() {
    ...
    fetch(`/procurement/purchaseOrder/plan/${val}/categories`)
        .then(r => r.json())
        .then(({success, data}) => {
...
function setDirectMode(enabled) {
    ...
    if (enabled) {
        ...
        Promise.all([
            fetch(`/procurement/purchase-order/root-categories`).then(r => r.json()).catch(() => ({data: []})),
            fetch(`/procurement/purchase-order/direct-plan-categories`).then(r => r.json()).catch(() => ({data: []})),
        ]).then(([root, direct]) => {
            const seen = new Set();
...
$('#itemCategory').on('change', function() {
    ...
    if (planId > 0) {
        ...
        console.log('Fetching suppliers from:', `/procurement/purchase-order/prequalified-suppliers/0`);
        fetch(`/procurement/purchase-order/prequalified-suppliers/0`)
            .then(r => {
    ...
    if (catId > 0 && planId > 0) {
        fetch(`/procurement/purchase-order/plan/${planId}/category/${catId}/items`)
            .then(r => r.json())
            .then((resp) => {
...
// Prequalified supplier selection
        ...
        fetch(`/procurement/purchase-order/direct-plan-items/${val}`)
            ...
            .then(({success, data, items}) => {
                ...
                if (rows && rows.length >= 0) {
                    ...
                    const $cat = $('#itemCategory');
                    $cat.prop('disabled', true).empty().append('<option value="" selected>-- None --</option>');
                    fetch(`/procurement/purchase-order/plan/${val}/categories`)
                        .then(r => r.json())
                        .then(({success, data}) => {
                            (data || []).forEach(row => {
                                $cat.append(`<option value="${row.Id}">${row.Name}</option>`);
