@extends('layouts.app')
@section('title','New Insurance Type')

@section('content')
<div class="container my-3">
  <div class="card shadow-sm rounded-3">
    <div class="card-header bg-light py-2 px-3"><h6 class="mb-0 text-muted">Create Insurance Type</h6></div>
    <div class="card-body p-3">
      @if($errors->any())<div class="alert alert-danger"><ul class="mb-0">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>@endif
      <form method="post" action="{{ route('assets.settings.insurance-types.store') }}">
        @csrf
        <div class="row g-3">
        ...
        <div class="mt-3"><a href="{{ route('assets.settings.insurance-types.index') }}" class="btn btn-outline-secondary btn-sm">Back</a><button class="btn btn-primary btn-sm ms-2">Save</button></div>
      </form>
    </div>
  </div>
</div>
