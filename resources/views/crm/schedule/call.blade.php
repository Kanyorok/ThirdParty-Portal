<div class="d-flex flex-column" style="height: 80%">
    ...
    @switch($schedule->ScheduleStatusID->value)
        ...
        @case(\App\Enums\ScheduleStatusEnum::PartialSuccess->value)
            ...
            <ul class="list-group list-group-flush">
                <li class="list-group-item">Status:
                    <span class="float-end">{{ $schedule->ScheduleStatusID->description() }}</span>
                </li>
                <li class="list-group-item">Start:
                    <span class="float-end">
                        {{ $call?->StartOn?->format('M d, Y h:ia') ?? 'N/A' }}
                    </span>
                </li>
                <li class="list-group-item">End:
                    <span class="float-end">
                        {{ $call?->EndOn?->format('M d, Y h:ia') ?? 'N/A' }}
                    </span>
                </li>
                <li class="list-group-item">Duration:
                    <span class="float-end">
                        @if($call?->StartOn && $call?->EndOn)
                            {{ $call->StartOn->diffForHumans($call->EndOn, \Carbon\CarbonInterface::DIFF_ABSOLUTE, short: true, parts: 2) }}
                        @else
                            N/A
                        @endif
                    </span>
        ...
        @case(\App\Enums\ScheduleStatusEnum::Canceled->value)
            ...
            <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item">Start:
                    <span class="float-end">{{ $schedule->StartOn->format('M d, Y h:ia') }}</span>
                </li>
                <li class="list-group-item">End:
                    <span class="float-end">{{ $schedule->EndOn->format('M d, Y h:ia') }}</span>
                </li>
                <li class="list-group-item">Duration:
                    <span class="float-end">
                        {{ $schedule->StartOn->diffForHumans($schedule->EndOn, \Carbon\CarbonInterface::DIFF_ABSOLUTE, short: true, parts: 2) }}
                    </span>
                </li>
            </ul>
            @break
        ...
        @case(\App\Enums\ScheduleStatusEnum::Scheduled->value)
            @if(now()->startOfDay()->gte($schedule->EndOn))
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item">Start: <span class="float-end">{{ $schedule->StartOn->format('M d, Y h:ia') }}</span></li>
                    <li class="list-group-item">End: <span class="float-end">{{ $schedule->EndOn->format('M d, Y h:ia') }}</span></li>
                    <li class="list-group-item">Duration: <span class="float-end">
                        {{ $schedule->StartOn->diffForHumans($schedule->EndOn, \Carbon\CarbonInterface::DIFF_ABSOLUTE, short: true, parts: 2) }}</span></li>
                </ul>
            @else
    ...
    <p class="mb-3">Schedule Notes:
        <i class="align-middle" data-feather="info"></i> <br>
            ...
            <div class="modal-body">
                <div class="onboarding-content text-center">
                    ...
                    <form id="deleteCallScheduleForm"
                        ...
                        @method('delete')
                        <div class="mt-4">
                            <button type="button" class="btn btn-success float-start" data-bs-dismiss="modal">
                                No, Keep
                            </button>
                            <button class="btn btn-danger float-end" id="deleteCallScheduleBtn" type="submit">
                                <i class="fas fa-trash"></i> Yes, Cancel Call
                            </button>
                        </div>
                    </form>
                </div>
