...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0">New Certificate Template</h2>
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('crm.training.certificate-templates.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea id="TemplateBody" name="TemplateBody" class="form-control" rows="6">{{ old('TemplateBody') }}</textarea>
                        <div class="form-text">Use placeholders from the reference card below.</div>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Default Issuing Body</label>
                        <input type="text" name="DefaultIssuingBody" class="form-control" value="{{ old('DefaultIssuingBody') }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Default Validity (Months)</label>
                        <input type="number" min="1" max="240" name="DefaultValidityMonths" class="form-control" value="{{ old('DefaultValidityMonths') }}">
                    </div>
                    <div class="col-md-3">
                        ...
                        <select id="BackgroundImagePath" name="BackgroundImagePath" class="form-select">
                            ...
                            @foreach($backgroundImageChoices as $path => $label)
                                <option value="{{ $path }}" data-image-url="{{ asset($path) }}" @selected(old('BackgroundImagePath') === $path)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" id="IsSample" name="IsSample" value="1" @checked(old('IsSample'))>
                            <label class="form-check-label" for="IsSample">Mark as sample</label>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" id="IsActive" name="IsActive" value="1" @checked(old('IsActive', true))>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if ((bodyInput.value || '').trim() === '') {
            bodyInput.value = 'This certifies that '
