            ...
            <form method="POST" action="{{ route('crm.training.programs.store') }}">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-9">
                        ...
                        <select name="TargetClients[]" class="form-select" multiple>
                            @foreach($clients as $client)
                                <option value="{{ $client->ClientID }}" @selected(in_array($client->ClientID, old('TargetClients', []), true))>
                                    {{ $client->Name }} ({{ $client->ClientID }})
                                </option>
                            @endforeach
                        </select>
                        <div class="form-text">Leave empty to keep the program open to manually enrolled clients.</div>
                    </div>
                </div>
