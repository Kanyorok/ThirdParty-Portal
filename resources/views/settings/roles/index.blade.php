        ...
        <div class="card">
            <div class="card-header">
                <div class="card-actions float-end">
                    <button class="btn btn-info ms-2" type="button" id="syncPermissionsBtn">
    ...
    $(function() {
        ...
        $(document).on('click', '.view-role-users', function(e) {
            ...
            $('#syncPermissionsBtn').on('click', function() {
                if (confirm('Are you sure you want to sync permissions? This will ensure all permissions (including dynamic ones) are assigned to the Admin role.')) {
                    const $btn = $(this);
                    const originalText = $btn.html();
                    $btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Syncing...');

                    $.post("{{ route('roles.seedPermissions') }}", {
                            _token: "{{ csrf_token() }}"
                        })
                        ...
                        .fail(function(xhr) {
                            nWarning(xhr.responseJSON?.message || 'Failed to sync permissions.');
                        })
                        .always(function() {
