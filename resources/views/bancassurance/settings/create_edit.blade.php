...
@section('content')
    <div class="container mt-4">
        ...
        <form method="POST"
              action="{{ isset($codeDetail) ? route('bancassurance.settings.update', $codeDetail->ID) : route('bancassurance.settings.store') }}">
            @csrf
            @if(isset($codeDetail))
                @method('PUT')
            @endif
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="CodeID" id="CodeID" class="form-select" required>
                        ...
                        @foreach($codeCategories as $category)
                            <option
                                value="{{ $category }}" {{ (old('CodeID', $codeDetail->CodeID ?? '') == $category) ? 'selected' : '' }}>
                                {{ $category }}
                            </option>
                        @endforeach
                    </select>
