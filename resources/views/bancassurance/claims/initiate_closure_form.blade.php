    ...
    <div class="container mt-4">
        ...
        <form method="POST" action="{{ route('bancassurance.claims.storeClosureFromList') }}">
                ...
                <select name="ClaimID" class="form-select" required>
                    ...
                    @foreach($claims as $claim)
                        <option value="{{ $claim->Id }}">
                            [#{{ $claim->Id }}] {{ $claim->ClaimType }} – Policy {{ $claim->PolicyNumber }}
                            – {{ $claim->CustomerName }} (KES {{ number_format($claim->ApprovalAmount, 2) }})
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="text-end">
                ...
                <a href="{{ route('bancassurance.claims.closed') }}" class="btn btn-secondary">⬅️ Back to Closed
                    Claims</a>
            </div>
        </form>
    </div>
