...
<form action="{{ route('bancassurance.claims.documents.upload', $claim->Id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="mb-3 d-none" id="otherTypeDiv">
        ...
        <input type="text" name="OtherType" id="OtherType" class="form-control" placeholder="Enter other document type">
    </div>

    <div class="mb-3">
        <label for="DocumentName" class="form-label">Document Name / Description</label>
        <input type="text" name="DocumentName" class="form-control" placeholder="e.g. Medical Report from Nairobi Hospital" required>
    </div>
        ...
        <ul class="list-group">
            @foreach($documents as $doc)
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ asset('storage/' . $doc->FilePath) }}" target="_blank" class="btn btn-sm btn-outline-primary">View</a>
                </li>
            @endforeach
        </ul>
    @else
