        ...
        <tbody>
            @forelse($logs as $log)
                <tr>
                    ...
                    <td>{{ $log->Notes }}</td>
                    <td>
                    <a href="{{ route('bancassurance.customers.communication.edit', $log->Id) }}" class="btn btn-sm btn-warning">Edit</a>
                    <form action="{{ route('bancassurance.customers.communication.destroy', $log->Id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
