    ...
    <form method="POST" action="{{ route('bancassurance.premiums.update', $payment->Id) }}">
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-3">
                ...
                <input type="hidden" name="CustomerID" id="customer-id" value="{{ old('CustomerID', $payment->CustomerID) }}">
            </div>
            <div class="col-md-3">
                <label class="form-label">Payment Frequency</label>
                <input type="text" id="payment-frequency-display" class="form-control" readonly>
                <input type="hidden" name="PaymentFrequency" id="payment-frequency-id" value="{{ old('PaymentFrequency', $payment->PaymentFrequency) }}">
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="PaymentMode" class="form-select">
                ...
                @foreach ($paymentModes as $paymentMode)
                    <option value="{{ $paymentMode->ID }}" {{ $payment->PaymentMode == $paymentMode->ID ? 'selected' : '' }}>
