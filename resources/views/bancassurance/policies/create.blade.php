        ...
        <form method="POST" action="{{ route('bancassurance.policies.store') }}">
            ...
            @if(isset($referral))
                <div class="alert alert-info">
                    <strong>Referral Linked:</strong> Based on Referral #{{ $referral->Id }} by Staff
                    ID: {{ $referral->ReferredBy }}
                </div>
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <input type="number" name="SumAssured" class="form-control" value="{{ old('SumAssured') }}"
                           required>
                </div>

                <div class="col-md-4">
                    <label class="form-label">Premium Amount</label>
                    <input type="number" name="PremiumAmount" class="form-control" value="{{ old('PremiumAmount') }}"
                           required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <select name="PaymentFrequency" class="form-select" required>
                        ...
                        @foreach($paymentfrequencys as $freq)
                            <option
                                value="{{ $freq->ID }}" {{ old('PaymentFrequency') == $freq->Id ? 'selected' : '' }}>
                                {{ $freq->Description }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="date" name="PolicyStartDate" class="form-control" value="{{ old('PolicyStartDate') }}"
                           required>
                </div>

                <div class="col-md-6">
                    <label class="form-label">Policy End Date</label>
                    <input type="date" name="PolicyEndDate" class="form-control" value="{{ old('PolicyEndDate') }}"
                           required>
                </div>
            </div>
