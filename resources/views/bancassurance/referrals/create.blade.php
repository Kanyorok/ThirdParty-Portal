        ...
        <form method="POST" action="{{ route('bancassurance.referrals.store') }}">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="ClientIDNumber" class="form-control" value="{{ old('ClientIDNumber') }}"
                           required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Phone <span class="text-danger">*</span></label>
                    <input type="text" name="ClientPhone" class="form-control" value="{{ old('ClientPhone') }}"
                           required>
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="email" name="ClientEmail" class="form-control" value="{{ old('ClientEmail') }}"
                           required>
                </div>
                <div class="col-md-6">
                    ...
                    <select name="InsuranceProductId" class="form-select" required>
                        ...
                        @foreach ($insuranceproducts as $product)
                            <option
                                value="{{ $product->ID }}" {{ old('InsuranceProductId') == $product->ID ? 'selected' : '' }}>
                                {{ $product->Description }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="PreferredInsurerId" class="form-select" required>
                        ...
                        @foreach ($insurers as $insurer)
                            <option
                                value="{{ $insurer->ID }}" {{ old('PreferredInsurerId') == $insurer->ID ? 'selected' : '' }}>
                                {{ $insurer->Description }}
                            </option>
                        @endforeach
                    </select>
