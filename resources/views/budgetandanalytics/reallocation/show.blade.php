...
<div class="container my-3">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm rounded-3">
                ...
                <div class="card-body small">
                    ...
                    <p><strong>Amount:</strong> <span class="fw-bold text-primary">{{ number_format($realloc->Amount,2) }}</span></p>
                    ...
                    <p><strong>Approved:</strong> {{ optional($realloc->ApprovedOn)->format('Y-m-d H:i') ?? 'Pending' }}</p>
                </div>
            </div>
        </div>
                                ...
                                <ul class="list-group list-group-flush small">
                                    @foreach($fromLimits as $lim)
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <span class="fw-bold text-danger">{{ number_format($lim->LimitAmount,2) }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            @else
                            ...
                            @if($toLimits->count())
                                <ul class="list-group list-group-flush small">
                                    @foreach($toLimits as $lim)
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            {{ \Carbon\Carbon::parse($lim->EffectiveFrom)->format('F') }}
                                            <span class="fw-bold text-success">{{ number_format($lim->LimitAmount,2) }}</span>
                                        </li>
                                    @endforeach
                                </ul>
                            @else
    ...
    <div class="mt-3 d-flex gap-2">
        @if(strtolower($realloc->Status) === 'pending')
            <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#decisionModal" data-action="approve">
                <i class="fas fa-check me-1"></i> Approve
            </button>
            <button class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#decisionModal" data-action="reject">
