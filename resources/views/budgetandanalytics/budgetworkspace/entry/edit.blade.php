...
<div class="card p-4">
    ...
    <form action="{{ route('budgetprojections.update', $budget->Id ?? '') }}" method="POST">
            ...
            <div class="col-md-6">
                ...
                <select name="CurrencyID" id="currency" class="form-select" required>
                    @foreach($currencies as $currency)
                        <option value="{{ $currency->Id }}" {{ (isset($budget->CurrencyID) && $budget->CurrencyID == $currency->Id) ? 'selected' : '' }}>{{ $currency->Code }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="mt-4">
            ...
            <div class="mt-2 mb-2">
                <button type="button" class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addProductModal">➕ Add Product Row</button>
            </div>
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle text-center" id="productsTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $i+1 }}</td>
                            <td>{{ $projection->productType->Name }}<input type="hidden" name="Products[{{ $i }}][ProductID]" value="{{ (int)$projection->ProductID }}"></td>
                            <td><input type="number" name="Products[{{ $i }}][Volume]" class="form-control" value="{{ $projection->Volume }}" min="0"></td>
                            <td><input type="number" step="0.01" name="Products[{{ $i }}][Value]" class="form-control" value="{{ $projection->Value }}" min="0"></td>
                ...
                <tbody>
                    ...
                    @foreach ($months as $i => $month)
                        <tr>
                            <td>{{ $month }}</td>
                            <td>
                                <input type="number" step="0.01" min="0" name="MonthlyAllocations[{{ $i+1 }}]" class="form-control" value="{{ isset($allocMap[$i+1]) ? ($allocMap[$i+1]->Allocation ?? $allocMap[$i+1]->Amount) : 0 }}">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
