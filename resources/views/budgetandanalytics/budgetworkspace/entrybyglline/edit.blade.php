  ...
  <form action="{{ route('entrybyglline.update', $entry->Id) }}" method="POST">
      ...
      <div class="col-md-6">
        ...
        <select class="form-select" name="BranchID" required>
          ...
          @foreach ($branches as $item)
            <option value="{{ $item->Id }}" {{$entry->BudgetID == $item->Id ? 'selected' : ''}}>{{ $item->Name }}</option>
          @endforeach
        </select>
      </div>
    </div>
    ...
    <div class="mb-3">
      ...
      <select name="BudgetLineID" class="form-select" required>
        ...
        @foreach ($budgetLines as $item)
          <option value="{{ $item->Id }}" {{$entry->BudgetLineID == $item->Id ? 'selected' : ''}}>{{ $item->LineName }}</option>
        @endforeach
      </select>
    ...
    <div class="manual-entry">
      <div class="mb-3">
        ...
        <input type="number" step="0.01" min="0.00" id="totalAmount" name="Amount" class="form-control" value="{{ $entry->Amount }}" required>
      </div>
    </div>
      ...
      <div class="row">
        ...
        @foreach($months as $key)
          <div class="col-md-3 mb-2">
            ...
            <input type="number" value="{{ old('monthly_allocations.'.$key, $entry->allocations->where('Month', str_pad($key, 2, '0', STR_PAD_LEFT))->first()->Allocation ?? 0) }}" min="0" name="monthly_allocations[{{ $key }}]" class="form-control" placeholder="0.00">
          </div>
        @endforeach
      </div>
