...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <div class="mb-2 d-flex justify-content-between">
            <a href="{{ route('budgetlinemapping.create') }}" class="btn btn-success btn-sm" data-bs-toggle="modal"
               data-bs-target="#addLineModal">
                + New Budget Line</a>
        </div>
                    ...
                    <form action="{{ route('budgetlinemapping.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select multiple class="form-select" name="GLS[]" required>
                                @foreach ($gls as $item)
                                    <option value="{{ $item->Id }}">GL00{{ $item->Id }}
    ...
    @foreach ($budgetLines as $line)
        ...
        <div class="modal fade" id="editLineModal{{ $line->Id }}" tabindex="-1"
             aria-labelledby="editLineModalLabel{{ $line->Id }}" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content rounded-3 shadow">
                    <div class="modal-header">
                    ...
                    <form action="{{ route('budgetlinemapping.update', $line->Id) }}" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label">Budget Line Name</label>
                                <input type="text" class="form-control" name="LineName" value="{{ $line->LineName }}"
                                       required>
                                @error('LineName')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                                ...
                                <select multiple class="form-select" name="GLS[]" required>
                                    ...
                                    @foreach ($gls as $item)
                                        <option value="{{ $item->Id }}"
                                                @if(in_array($item->Id, $selectedGLs)) selected @endif>
                            ...
                            <!-- Primary Checkbox -->
                            <div class="form-check mb-3">
                                <input class="form-check-input" name="IsDefault" type="checkbox"
                                       id="primaryCheck{{ $line->Id }}" @checked($line->IsDefault)>
