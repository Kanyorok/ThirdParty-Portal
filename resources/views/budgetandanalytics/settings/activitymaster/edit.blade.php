...
<div class="card p-4">
    ...
    <form action="{{ route('activitymaster.update', $activity->Id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="BudgetLineID" id="BudgetLineID" class="form-select" required>
                ...
                @foreach($lines as $line)
                    <option value="{{ $line->Id }}" {{ old('BudgetLineID', $activity->BudgetLineID) == $line->Id ? 'selected' : '' }}>
                        {{ $line->LineName }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="ActivityName" id="ActivityName" class="form-control" value="{{ old('ActivityName', $activity->ActivityName) }}" required>
        </div>
        ...
        <div class="form-check mb-3">
            <input type="checkbox" name="IsActive" id="IsActive" class="form-check-input" value="0" {{ old('IsActive', $activity->IsActive) ? 'checked' : '' }}>
