            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="VehicleType" required>
                    ...
                    @foreach ($vehicleTypes as $type)
                        <option value="{{ $type->ID }}" {{ old('VehicleType', $vehicle->VehicleType) == $type->ID ? 'selected' : '' }}>
                            {{ $type->Description }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="FuelType">
                    ...
                    @foreach ($fuelTypes as $fuel)
                        <option value="{{ $fuel->Id }}" {{ old('FuelType', $vehicle->FuelType) == $fuel->Id ? 'selected' : '' }}>
                            {{ $fuel->FuelName }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-control" id="make" name="Make" required>
                    ...
                    @foreach($brands as $brand)
                        <option value="{{ $brand->Id }}" {{ old('Make', $vehicle->Make) == $brand->Id ? 'selected' : '' }}>
                            {{ $brand->BrandName }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="Status" required>
                    ...
                    @foreach ($vehicleStatuses as $status)
                        <option value="{{ $status->ID }}" {{ old('Status', $vehicle->Status) == $status->ID ? 'selected' : '' }}>
                            {{ $status->Description }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="AssignedBranch">
                    ...
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->Id }}" {{ old('AssignedBranch', $vehicle->AssignedBranch) == $branch->Id ? 'selected' : '' }}>
                            {{ $branch->Name }}
                        </option>
                    @endforeach
                </select>
