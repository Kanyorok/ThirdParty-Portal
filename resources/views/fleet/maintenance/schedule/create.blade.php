...
<div class="card p-4 shadow rounded-4">
    <form method="POST" action="{{ isset($schedule) ? route('fleet.maintenance_schedule.update', $schedule->Id) : route('fleet.maintenance_schedule.store') }}">
        @csrf
        @if(isset($schedule))
            @method('PUT')
        @endif
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <select name="VehicleID" id="VehicleID" class="form-select" required>
                    ...
                    @foreach($vehicles as $vehicle)
                        <option value="{{ $vehicle->Id }}" {{ (isset($schedule) && $schedule->VehicleID == $vehicle->Id) ? 'selected' : '' }}>
            ...
            <div class="col-md-6">
                ...
                <select name="MaintenanceType" id="MaintenanceType" class="form-select" required>
                    ...
                    @foreach($maintenanceType as $type)
                        <option value="{{ $type->ID }}" {{ (isset($schedule) && $schedule->MaintenanceType == $type->ID) ? 'selected' : '' }}>
                            {{ $type->Description }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-12">
                ...
                <textarea name="Notes" id="Notes" class="form-control" rows="3">{{ $schedule->Notes ?? '' }}</textarea>
            </div>
