...
@section('content')
    <div class="card shadow p-4 rounded-4">
        ...
        <form action="{{ route('fleet.insurance_tracker.update', $record->Id) }}" method="POST"
            ...
            <div class="mb-3">
                ...
                <select name="VehicleID" id="VehicleID" class="form-select" required>
                    ...
                    @foreach($vehicles as $v)
                        <option
                            value="{{ $v->Id }}" {{ old('VehicleID', $record->VehicleID) == $v->Id ? 'selected' : '' }}>
                            {{ $v->RegistrationNo }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="InsuranceProvider" required>
                    ...
                    @foreach ($insuranceProvider as $provider)
                        <option
                            value="{{ $provider->Id }}" {{ old('InsuranceProvider', $record->InsuranceProvider) == $provider->Id ? 'selected' : '' }}>
                            {{ $provider->Name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control" name="PremiumAmount"
                       value="{{ old('PremiumAmount', $record->PremiumAmount) }}" required>
            </div>

            <div class="mb-3">
                <label for="CoverageStartDate" class="form-label">Start Date</label>
                <input type="date" class="form-control" name="CoverageStartDate"
                       value="{{ old('CoverageStartDate', $record->CoverageStartDate) }}" required>
            </div>

            <div class="mb-3">
                <label for="CoverageEndDate" class="form-label">Expiry Date</label>
                <input type="date" class="form-control" name="CoverageEndDate"
                       value="{{ old('CoverageEndDate', $record->CoverageEndDate) }}" required>
            </div>

            <div class="mb-3">
                <label for="RenewalReminderDate" class="form-label">Renewal Reminder Date</label>
                <input type="date" class="form-control" name="RenewalReminderDate"
                       value="{{ old('RenewalReminderDate', $record->RenewalReminderDate) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="Status" required>
                    ...
                    @foreach ($insuranceStatuses as $status)
                        <option
                            value="{{ $status->ID }}" {{ old('Status', $record->Status) == $status->ID ? 'selected' : '' }}>
                            {{ $status->Description }}
                        </option>
                    @endforeach
                </select>
