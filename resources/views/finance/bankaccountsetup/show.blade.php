        ...
        <div class="card shadow-sm rounded-4 border-0">
            <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex gap-2">
                    <a href="{{ route('finance.bankaccountsetup.edit', $account->AccountID) }}"
                        ...
                        <dl class="row mb-0">
                            ...
                            <dd class="col-7">
                                @if($account->currency)
                                    ...
                                    — {{ $account->currency->Name }} {{ $account->currency->Symbol ? '(' . $account->currency->Symbol . ')' : '' }}
                                @else
                                    —
                                @endif
                            </dd>
            ...
            <div class="card-footer bg-white d-flex justify-content-between align-items-center">
                <small class="text-muted">Created {{ $account->CreatedOn }} •
                    Modified {{ $account->ModifiedOn ?? '—' }}</small>
                <a href="{{ route('finance.bankaccountsetup.index') }}" class="btn btn-outline-secondary btn-sm"><i
