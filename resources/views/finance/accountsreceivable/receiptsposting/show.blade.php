                        ...
                        <div class="col-lg-5">
                            <div class="border rounded-3 p-3 h-100 d-flex flex-column">
                                ...
                                <div class="mt-auto small text-muted pt-2">
                                    Please quote receipt number <strong>{{ $receipt['receiptNo'] }}</strong> on all correspondence.
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border rounded-3 p-2 small print-only">
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                ...
                                <div><strong>Amount:</strong> {{ $receipt['currencySymbol'] }} {{ number_format($receipt['amountReceived'],2) }}</div>
            ...
            <div class="col-12">
                <div id="receiptSlip" class="card shadow-sm border-0 p-3 rounded-4 d-none">
                    <div class="slip">
                        ...
                        <div class="slip-row slip-bold"><span>Amount Received</span><span>{{ $receipt['currencySymbol'] }} {{ number_format($receipt['amountReceived'],2) }}</span></div>
                        <div class="slip-row"><span>Applied</span><span>{{ $receipt['currencySymbol'] }} {{ number_format($appliedTotal,2) }}</span></div>
                        @if($unapplied > 0)
                            <div class="slip-row"><span>Unapplied</span><span>{{ $receipt['currencySymbol'] }} {{ number_format($unapplied,2) }}</span></div>
                        @endif
                        <div class="slip-sep"></div>

                        <div class="slip-sub">Invoices</div>
                        @foreach($allocations as $row)
                            <div class="slip-row"><span>{{ $row['invoiceNo'] }}</span><span>{{ $receipt['currencySymbol'] }} {{ number_format($row['applied'],2) }}</span></div>
                        @endforeach
                        <div class="slip-sep"></div>
...
@section('styles')
    <style>
        ...
        .slip-sub { text-align: center; color: #6c757d; font-size: 12px; }
        .slip-sep { border-top: 1px dashed #999; margin: 6px 0; }
