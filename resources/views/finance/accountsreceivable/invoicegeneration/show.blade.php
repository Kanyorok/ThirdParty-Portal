        ...
        <div class="p-3 p-md-4 border-bottom bg-light rounded-top-4"
            ...
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <img src="{{asset('assets/img/icons/craft-logo.png')}}" alt="Logo" style="height:38px"
                ...
                <div class="text-md-end">
                    ...
                    <span
                        class="badge rounded-pill text-bg-secondary px-3 py-2">{{ $invoice->currency->Code ?? '' }}</span>
                    @php
                        $statusClasses = [
                    ...
                    <div class="mt-1 h5 mb-0 fw-semibold">Invoice: <span
                            class="text-nowrap">{{ $invoice->InvoiceNumber }}</span></div>
                    ...
                    <div class="card h-100 border-0 shadow-sm rounded-4">
                        <div class="card-body py-3">
                            ...
                            <div class="small text-muted mt-2">Email: <span
                                    class="text-dark">{{ $invoice->customer->EmailAddress ?? '—' }}</span></div>
                            <div class="small text-muted">Phone: <span
                                    class="text-dark">{{ $invoice->customer->PhoneNumber ?? '—' }}</span></div>
                            <div class="small text-muted">ID/Reg: <span
                                    class="text-dark">{{ $invoice->customer->IDRegistrationNo ?? '—' }}</span></div>
                            <div class="small text-muted">Nationality: <span
                                    class="text-dark">{{ $invoice->customer->Nationality ?? '—' }}</span></div>
                ...
                <div class="col-lg-4">
                    <div class="card h-100 border-0 shadow-sm rounded-4">
                        <div class="card-body py-3">
                            ...
                            <div class="small">Craft Silicon Campus, Musa Gitau Road,<br>off Waiyaki Way, Nairobi, Kenya
                            </div>
            ...
            <div class="row g-3 mt-2 small">
                <div class="col-md-6">
                    <div class="border rounded-3 p-2">
                        ...
                        <div
                            class="fw-medium">{{ \Carbon\Carbon::parse($invoice->CreatedOn)->format('Y-m-d H:i') }}</div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="border rounded-3 p-2">
                        ...
                        <div
                            class="fw-medium">{{ \Carbon\Carbon::parse($invoice->ModifiedOn)->format('Y-m-d H:i') }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="border rounded-3 p-2 print-only small">
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        ...
                        <div>
                            <strong>Amount:</strong> {{ $invoice->currency->Symbol }} {{ number_format($invoice->TotalAmount,2) }}
                        </div>
    ...
    <div class="modal fade" id="approveModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content rounded-4">
                ...
                <form action="{{ route('ar.invoice.approve', $invoice->Id ?? $invoice->id) }}" method="POST"
                      id="approveForm">
                    @csrf
                    @method('POST')
                    <div class="modal-body">
                        <div class="alert alert-info small">
                            ...
                            Amount: {{ $invoice->currency->Symbol }} {{ number_format($invoice->TotalAmount,2) }}
                        </div>
                        <div class="mb-3">
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="btn btn-success" type="submit"><i class="fas fa-check-circle"></i> Approve
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div class="modal fade" id="rejectModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content rounded-4">
                ...
                <form action="{{ route('ar.invoice.reject', $invoice->Id ?? $invoice->id) }}" method="POST"
                      id="approveForm">
                    @csrf
                    @method('POST')
                    <div class="modal-body">
