...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="btn-group">
            ...
            <a href="{{ route('finance.cheques.received.create') }}" class="btn btn-outline-success">New Received
                Cheque</a>
        </div>
    </div>
            ...
            @forelse($rows as $r)
                <tr>
                    ...
                    <td>
                        ...
                        @if($r->chequeBook)
                            <small
                                class="text-muted d-block">Book: {{ $r->chequeBook->BookName ?? $r->chequeBook->ChequeBookID }}</small>
                        @endif
                    </td>
                    ...
                    <td>{{ $r->PartyName ?? '-' }}</td>
                    <td><span
                            class="badge bg-{{ $r->Status==='Cleared'?'success':($r->Status==='Bounced'?'danger':($r->Status==='Deposited'?'info':($r->Status==='OnHand'?'warning text-dark':($r->Status==='Issued'?'secondary':'dark')))) }}">{{ $r->Status }}</span>
                    </td>
                    <td class="text-end">
                        <a href="{{ route('finance.cheques.show',$r->ChequeID) }}"
                           class="btn btn-sm btn-outline-primary">Open</a>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="9" class="text-center text-muted">No cheques.</td>
                </tr>
            @endforelse
            </tbody>
        </table>
