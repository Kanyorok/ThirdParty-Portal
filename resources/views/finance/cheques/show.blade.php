...
<div class="card mb-3">
  <div class="card-body">
    <div class="row g-2">
      <div class="col-md-3"><strong>Status:</strong> <span class="badge bg-{{ $row->Status==='Cleared'?'success':($row->Status==='Bounced'?'danger':($row->Status==='Deposited'?'info':($row->Status==='OnHand'?'warning text-dark':($row->Status==='Issued'?'secondary':'dark')))) }}">{{ $row->Status }}</span></div>
      <div class="col-md-3"><strong>Cheque No:</strong> {{ $row->ChequeNumber }}</div>
      <div class="col-md-3"><strong>Cheque Date:</strong> {{ $row->ChequeDate ? \Illuminate\Support\Carbon::parse($row->ChequeDate)->format('Y-m-d') : '-' }}</div>
      <div class="col-md-3"><strong>Due Date:</strong> {{ $row->DueDate ? \Illuminate\Support\Carbon::parse($row->DueDate)->format('Y-m-d') : '-' }}</div>
      ...
      <div class="col-md-4"><strong>Party:</strong> {{ $row->PartyType ?? '-' }} {{ $row->PartyID ? '#'.$row->PartyID : '' }}</div>
...
<div class="card">
  <div class="card-body">
    ...
    @if($row->Direction==='RECEIVED' && in_array($row->Status,['OnHand']))
      {{-- Deposit Form --}}
      <form method="POST" action="{{ route('finance.cheques.deposit',$row->ChequeID) }}" class="row g-2 align-items-end border rounded p-3 mb-3">
        @csrf
        <div class="col-md-4">
    ...
    @if( ($row->Direction==='ISSUED' && in_array($row->Status,['Issued'])) ||
      ...
      <form method="POST" action="{{ route('finance.cheques.clear',$row->ChequeID) }}" class="row g-2 align-items-end border rounded p-3 mb-3">
        @csrf
        <div class="col-md-3">
    ...
    @if( ($row->Direction==='RECEIVED' && in_array($row->Status,['Deposited'])) ||
      ...
      <form method="POST" action="{{ route('finance.cheques.bounce',$row->ChequeID) }}" class="row g-2 align-items-end border rounded p-3 mb-3">
        @csrf
        <div class="col-md-3">
        ...
        <div class="col-md-5">
          ...
          <input name="Reason" class="form-control" placeholder="e.g., Refer to Drawer, Insufficient funds">
        </div>
        <div class="col-md-3">
    ...
    @if(in_array($row->Status,['Draft','Issued','OnHand']))
      <form method="POST" action="{{ route('finance.cheques.cancel',$row->ChequeID) }}" onsubmit="return confirm('Cancel this cheque?');">
        @csrf
