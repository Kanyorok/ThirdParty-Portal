        ...
        <table class="table table-sm table-striped align-middle">
            ...
            @forelse($rows as $b)
                <tr>
                    ...
                    <td>{{ $b->BookName ?? '-' }} <small class="text-muted">{{ $b->Prefix }}..{{ $b->Suffix }}</small>
                    </td>
                    ...
                    <td>
                        <span
                            class="badge bg-{{ $b->IsActive ? 'success':'secondary' }}">{{ $b->IsActive ? 'Yes':'No' }}</span>
                    </td>
                    <td class="text-end">
                        <a href="{{ route('finance.chequebooks.edit',$b->ChequeBookID) }}"
                           class="btn btn-sm btn-outline-primary">Edit</a>
                        <form action="{{ route('finance.chequebooks.destroy',$b->ChequeBookID) }}" method="POST"
                              class="d-inline" onsubmit="return confirm('Delete this cheque book?');">
                            @csrf @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="9" class="text-center text-muted">No cheque books yet.</td>
                </tr>
            @endforelse
            </tbody>
        </table>
