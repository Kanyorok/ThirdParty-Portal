...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <div class="d-flex gap-2">
    ...
    <a href="{{ route('finance.chequebooks.edit',$row->ChequeBookID) }}" class="btn btn-outline-primary">Edit</a>
  </div>
</div>
        ...
        <tbody>
          ...
          @forelse($leaves as $leaf)
            <tr>
              ...
              <td><span class="badge bg-{{ $leaf->Status==='Unused'?'secondary':($leaf->Status==='Issued'?'warning text-dark':($leaf->Status==='Cleared'?'success':($leaf->Status==='Bounced'?'danger':($leaf->Status==='Cancelled'?'dark':'info')))) }}">{{ $leaf->Status }}</span></td>
              <td>{{ $leaf->ChequeID ?? '-' }}</td>
              <td>{{ $leaf->UsedOn ?? '-' }}</td>
              ...
              <td class="text-end">
                @if($leaf->Status==='Unused')
                  <form action="{{ route('finance.chequebooks.leaves.spoil',[$row->ChequeBookID,$leaf->LeafID]) }}" method="POST" class="d-inline">
                    @csrf
                    <button class="btn btn-sm btn-outline-danger" onclick="return confirm('Mark this unused leaf as spoiled?');">Spoil</button>
                  </form>
                @endif
              </td>
            </tr>
          @empty
            <tr><td colspan="9" class="text-center text-muted">No leaves.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
