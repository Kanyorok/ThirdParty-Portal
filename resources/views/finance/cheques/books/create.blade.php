...
<form method="POST" action="{{ route('finance.chequebooks.store') }}">
  ...
  <div class="row g-3">
    ...
    <div class="col-md-5">
      ...
      <input name="BookName" class="form-control" value="{{ old('BookName') }}" placeholder="e.g., Main Ops Book 2025-Q3">
    </div>
    ...
    <div class="col-md-2">
      ...
      <input name="StartNumber" id="StartNumber" class="form-control" value="{{ old('StartNumber') }}" readonly>
    </div>
    <div class="col-md-2">
  ...
  <small class="text-muted d-block mt-2" id="rangeHelp">Select a bank account & size to see the proposed range.</small>
...
(function(){
  ...
  async function refreshRange(){
    ...
    if(!bankId) { startInp.value=''; endInp.value=''; nextInp.value=''; help.textContent='Select a bank account & size to see the proposed range.'; return; }
    try{
