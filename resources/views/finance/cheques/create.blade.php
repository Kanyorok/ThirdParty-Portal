...
<div class="row g-3">
    ...
    <select name="ChequeBookID" id="ChequeBookID" class="form-select" required>
      ...
      @foreach($books as $b)
        <option value="{{ $b->ChequeBookID }}"
                ...
                data-next="{{ $b->NextLeafNumber }}" data-start="{{ $b->StartNumber }}" data-end="{{ $b->EndNumber }}"
                @selected(old('ChequeBookID')==$b->ChequeBookID)>
          {{ optional($b->bankAccount->bank)->BankName }} — {{ $b->bankAccount?->AccountNumber }} ({{ $b->BookName ?? 'Book '.$b->ChequeBookID }})
        </option>
      @endforeach
    </select>
  ...
  <div class="col-md-4">
    ...
    <input name="ChequeNumber" id="ChequeNumber" class="form-control" value="{{ old('ChequeNumber') }}" required>
...
(function(){
  ...
  function suggest(){
    ...
    if(next){ if(!inp.value) inp.value = (pre?pre:'') + next + (suf?suf:''); }
