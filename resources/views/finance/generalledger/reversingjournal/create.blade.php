                ...
                <form method="POST" action="{{route('reversingjournal.store')}}" id="reverseJournalForm">
                    ...
                    <div class="row g-4 mb-3">
                        <div class="col-md-4">
                            ...
                            <select name="OriginalJournalID" id="OriginalReferenceNumber" class="form-select je-ref-select" required>
                                <option value="" disabled selected>-- Select Journal Ref --</option>
                                @foreach ($journalEntries as $entry)
                                    <option value="{{ $entry->Id }}" data-ref="{{ $entry->RefNo }}" data-desc="{{ $entry->Description ?? 'No Description' }}">{{ $entry->RefNo }} - {{ $entry->Description ?? 'No Description' }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <button class="btn btn-danger" id="openReverseModalBtn" type="button" data-bs-toggle="modal" data-bs-target="#reverseConfirmModal">Reverse Journal</button>
                    </div>
                </form>

                <!-- Confirmation Modal -->
                <div class="modal fade" id="reverseConfirmModal" tabindex="-1" aria-labelledby="reverseConfirmLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content rounded-4 shadow">
                            <div class="modal-header bg-light border-0">
                                <h5 class="modal-title text-danger" id="reverseConfirmLabel"><i class="fas fa-exclamation-triangle me-2"></i>Confirm Reverse Journal</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="alert alert-warning small">
                                    You are about to reverse the selected journal entry. This will create reversing lines and cannot be undone easily.
                                </div>
                                <div class="small">
