...
@section('styles')
    ...
    <style>
        ...
        .je-table .form-select, .je-table .form-control, .je-table textarea {
    ...
    <div class="container mt-0">
        <div class="card shadow rounded-4">
            <div class="card-header bg-light py-2 px-3 d-flex align-items-center">
                <h6 class="mb-0 text-muted"><i class="fas fa-sync-alt text-info me-1"></i>
                    ...
                    <div class="row mb-4 g-3">
                        <div class="col-md-3">
                            ...
                            <input type="date" name="StartDate" class="form-control"
                                   value="{{ optional($rec)->StartDate ? \Carbon\Carbon::parse(optional($rec)->StartDate)->format('Y-m-d') : date('Y-m-d') }}"
                                   required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Cutoff Date</label>
                            <input type="date" name="CuttOffDate" class="form-control"
                                   value="{{ optional($rec)->CuttOffDate ? \Carbon\Carbon::parse(optional($rec)->CuttOffDate)->format('Y-m-d') : date('Y-m-d') }}"
                                   required>
                        </div>
                        <div class="col-md-3">
                            ...
                            <select name="Frequency" class="form-select" required>
                                @foreach($paymentFrequency as $pf)
                                    <option
                                        value="{{ $pf->Value }}" {{ (optional($rec)->Frequency ?? '') === $pf->Value ? 'selected' : '' }}>{{ $pf->Description }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="text" name="ReferenceName" class="form-control"
                                   value="{{ optional($rec)->ReferenceName }}" required>
                        </div>
                            ...
                            @foreach($journalEntry->journalLines as $i => $line)
                                <tr>
                                    ...
                                    <td class="col-amount">
                                        ...
                                        <input type="number" name="Amount[]" class="form-control text-end" step="0.01"
                                               value="{{ number_format((float)$amount, 2, '.', '') }}" required>
                                    </td>
                                    <td class="col-narr">
                                        <textarea name="Narration[]"
                                                  class="form-control narration-input">{{ $line->Narration }}</textarea>
                                    </td>
