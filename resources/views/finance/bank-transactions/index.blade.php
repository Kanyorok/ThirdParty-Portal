        ...
        <table class="table table-sm table-striped align-middle">
            ...
            @forelse($rows as $r)
                <tr>
                    ...
                    <td>
          <span
              class="badge bg-{{ $r->Status === 'Posted' ? 'success' : ($r->Status === 'Draft' ? 'secondary' : 'danger') }}">
            {{ $r->Status }}
          </span>
                    </td>
                    <td class="text-end">
                        <a href="{{ route('finance.banktransactions.show',$r->BankTxnID) }}"
                           class="btn btn-sm btn-outline-primary">Open</a>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center text-muted">No bank transactions yet.</td>
                </tr>
            @endforelse
            </tbody>
        </table>
