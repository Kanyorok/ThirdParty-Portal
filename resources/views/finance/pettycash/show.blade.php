...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    ...
    <div class="d-flex gap-2">
      <span class="badge bg-{{ $row->Status==='Posted' ? 'success' : ($row->Status==='Voided' ? 'secondary' : 'warning text-dark') }}">
        {{ $row->Status }}
      </span>
...
<div class="card mb-3"><div class="card-body">
  <div class="row g-2">
    ...
    @if($row->SubmittedOn || $row->ApprovedOn || $row->PostedOn || $row->VoidedOn)
      ...
      @if($row->SubmittedOn)<div class="col-md-3"><small class="text-muted">Submitted:</small> {{ $row->SubmittedOn }}</div>@endif
      @if($row->ApprovedOn)<div class="col-md-3"><small class="text-muted">Approved:</small> {{ $row->ApprovedOn }}</div>@endif
      @if($row->PostedOn)<div class="col-md-3"><small class="text-muted">Posted:</small> {{ $row->PostedOn }}</div>@endif
      @if($row->VoidedOn)<div class="col-md-3"><small class="text-muted">Voided:</small> {{ $row->VoidedOn }}</div>@endif
    @endif
    ...
    <div class="table-responsive">
      <table class="table table-sm table-striped">
        <thead class="table-light">
          <tr><th>#</th><th>GL</th><th>Description</th><th class="text-end">Amount</th></tr>
        </thead>
        <tbody>
          ...
          @empty
            <tr><td colspan="4" class="text-center text-muted">No lines.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
...
<div class="card"><div class="card-body">
  ...
  @if($row->Status==='Draft')
    ...
    @if($mustApprove && !in_array($row->ApprovalStatus,['Pending','Approved']))
      @if(Route::has('finance.pettycash.submit'))
        <form method="POST" action="{{ route('finance.pettycash.submit',$row->VoucherID) }}" class="d-inline">@csrf
    ...
    @if($row->ApprovalStatus==='Pending')
      @if(Route::has('finance.pettycash.approve'))
        <form method="POST" action="{{ route('finance.pettycash.approve',$row->VoucherID) }}" class="d-inline">@csrf
          <button class="btn btn-success">Approve</button>
        </form>
      @endif
      @if(Route::has('finance.pettycash.reject'))
        <form method="POST" action="{{ route('finance.pettycash.reject',$row->VoucherID) }}" class="d-inline">@csrf
    ...
    @if(!$mustApprove || $row->ApprovalStatus==='Approved' || $row->ApprovalStatus==='N/A')
      <form method="POST" action="{{ route('finance.pettycash.post',$row->VoucherID) }}" class="d-inline">@csrf
        <button class="btn btn-primary">Post</button>
      </form>
    @endif

    {{-- Delete --}}
    <form method="POST" action="{{ route('finance.pettycash.destroy',$row->VoucherID) }}" class="d-inline" onsubmit="return confirm('Delete this voucher?');">
      @csrf @method('DELETE')
  ...
  @if($row->Status==='Posted')
    <form method="POST" action="{{ route('finance.pettycash.void',$row->VoucherID) }}" class="d-inline" onsubmit="return confirm('Void this voucher?');">@csrf
