...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="btn-group">
            <a class="btn btn-outline-primary" href="{{ route('finance.pettycash.disbursement.create') }}">New
                Disbursement</a>
    ...
    <form method="GET" class="row g-2 mb-3">
        <div class="col-md-3">
            ...
            <select name="FloatID" class="form-select" onchange="this.form.submit()">
                ...
                @foreach($floats as $f)
                    <option value="{{ $f->FloatID }}" @selected(request('FloatID')==$f->FloatID)>{{ $f->Name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-3">
              ...
              {{ $warn ? 'Needs Top-up' : 'OK' }}
                                ...
                                <div class="mt-3 d-flex gap-2">
                                    <a class="btn btn-sm btn-outline-primary"
                                       href="{{ route('finance.pettycash.replenishment.create') }}">New
                                        Replenishment</a>
        ...
        <table class="table table-sm table-striped">
            ...
            @forelse($rows as $r)
                <tr>
                    ...
                    <td><span
                            class="badge bg-{{ $r->Status==='Posted'?'success':($r->Status==='Voided'?'secondary':'warning text-dark') }}">{{ $r->Status }}</span>
                    </td>
                    <td class="text-end"><a class="btn btn-sm btn-outline-primary"
                                            href="{{ route('finance.pettycash.show',$r->VoucherID) }}">Open</a></td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center text-muted">No vouchers.</td>
                </tr>
            @endforelse
            </tbody>
        </table>
