    ...
    <div class="container my-3">
        <div class="card shadow-sm rounded-3" style="margin: 0.5rem;">
            <div class="card-header bg-light py-2 px-3 d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-0 text-primary"><i class="fas fa-code-branch me-2"></i> Branches
                        – {{ $bank->BankName }}</h5>
                    <small class="text-muted">Code: {{ $bank->BankCode ?? '—' }} · SWIFT: {{ $bank->SwiftCode ?? '—' }}
                        · CountryID: {{ $bank->CountryID ?? '—' }}</small>
                </div>
                <div>
                    <a href="{{ route('finance.bankbranch.create', ['bankId' => $bank->BankID]) }}"
                       class="btn btn-info btn-sm p-2">
                        <i class="fas fa-plus me-1"></i> New Branch
                    </a>
                    <a href="{{ route('finance.bank.index') }}" class="btn btn-outline-secondary btn-sm p-2">Back to
                        Banks</a>
                </div>
            </div>
            <div class="card-body p-3">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover align-middle">
                        ...
                        @forelse($branches as $branch)
                            <tr>
                                ...
                                <td class="text-nowrap">
                                    <a href="{{ route('finance.bankbranch.show', $branch->BranchID) }}"
                                       class="btn btn-sm btn-outline-info me-1" title="View"><i class="fas fa-eye"></i></a>
                                    <a href="{{ route('finance.bankbranch.edit', $branch->BranchID) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="text-center text-muted py-4">No branches yet.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
