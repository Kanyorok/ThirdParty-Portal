        ...
        <div class="card shadow-sm rounded-3" style="margin: 0.5rem;">
            <div class="card-header bg-light py-2 px-3 d-flex justify-content-between align-items-center">
                ...
                <div>
                    ...
                    <a href="{{ route('finance.bankbranch.index') }}" class="btn btn-outline-primary btn-sm p-2">All
                        Branches</a>
                </div>
            </div>
                    ...
                    <table class="table table-bordered table-hover align-middle">
                        ...
                        @forelse($banks as $bank)
                            <tr>
                                ...
                                <td>{{ $bank->Phone ?? '—' }}</td>
                                <td>
                                    <a href="{{ route('finance.bankbranch.bybank', $bank->BankID) }}"
                                       class="btn btn-sm btn-outline-secondary me-1">Branches</a>
                                </td>
                                <td>
                                    @if($bank->Website)
                                        <a href="{{ $bank->Website }}" target="_blank"
                                           rel="noopener">{{ $bank->Website }}</a>
                                    @else
                                        —
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="15" class="text-center text-muted py-4">No banks yet.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
