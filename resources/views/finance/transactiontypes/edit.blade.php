...
<div class="container mt-4">
    ...
    <form method="POST" action="{{ route('transactiontypes.update', $type->Id) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="Description" name="Description" value="{{ $type->Description }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="Module" id="Module" required>
                ...
                <option value="Procurement" {{ $type->Module == 'Procurement' ? 'selected' : '' }}>Procurement</option>
                <option value="Inventory" {{ $type->Module == 'Inventory' ? 'selected' : '' }}>Inventory</option>
                <option value="FixedAssets" {{ $type->Module == 'FixedAssets' ? 'selected' : '' }}>Fixed Assets</option>
