    ...
    <div class="card-body">
        ...
        <form action="{{ route('invoiceentry.update', $invoice->Id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-6">
                    ...
                    <select name="CurrencyID" class="form-select" required>
                        ...
                        @foreach($currencies as $currency)
                            <option value="{{ $currency->Id }}" {{ $invoice->CurrencyID == $currency->Id ? 'selected' : '' }}>
                                {{ $currency->Code }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <select id="POReference" class="form-select" name="POReference" style="flex: 1;" required>
                            ...
                            @foreach($orders as $po)
                                <option value="{{ $po->Id }}" {{ $invoice->POReference == $po->Id ? 'selected' : '' }}>
                                    {{ $po->OrderNo }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    ...
                    <select id="GRNReference" name="GRNReference" class="form-select" required>
                        ...
                        @foreach($grns as $grn)
                            <option value="{{ $grn->Id }}" {{ $invoice->GRNReference == $grn->Id ? 'selected' : '' }}>
                                {{ $grn->GRNID }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3">
                ...
                @if($invoice->FilePath)
                    <p class="mt-2">Current file: <a href="{{ asset('storage/'.$invoice->FilePath) }}" target="_blank">View File</a></p>
                @endif
            </div>
