        ...
        <div class="card-header bg-light py-2 px-3 d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-2">
                @if($invoice->Status === 'Draft')
                    <a href="{{ route('invoiceentry.edit', $invoice->Id) }}" class="btn btn-sm btn-outline-secondary">
                ...
                <div class="col-md-6">
                    <div class="border rounded-3 p-3 h-100">
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-7">{{ $invoice->InvoiceDate ? \Carbon\Carbon::parse($invoice->InvoiceDate)->format('d M Y') : '—' }}</div>
                        </div>

                        <div class="row mb-2">
                            <div class="col-5"><strong>Due Date:</strong></div>
                            <div class="col-7">{{ $invoice->DueDate ? \Carbon\Carbon::parse($invoice->DueDate)->format('d M Y') : '—' }}</div>
                        </div>
                        ...
                        <div class="row small text-muted">
                            <div class="col-md-6">
                                <div class="row mb-1">
                                    ...
                                    <div class="col-8">{{ $invoice->CreatedOn ? \Carbon\Carbon::parse($invoice->CreatedOn)->format('d M Y H:i') : '—' }}</div>
                                </div>
                                @if($invoice->creator)
                                    <div class="row mb-1">
                                        <div class="col-4">Created By:</div>
                                        <div class="col-8">{{ $invoice->creator->FirstName }} {{ $invoice->creator->LastName }}</div>
                                    </div>
                                @endif
                            </div>
                            <div class="col-md-6">
                                @if($invoice->ModifiedOn)
                                    <div class="row mb-1">
                                        ...
                                        <div class="col-8">{{ \Carbon\Carbon::parse($invoice->ModifiedOn)->format('d M Y H:i') }}</div>
                                    </div>
                                @endif
                                @if($invoice->modifier)
                                    <div class="row mb-1">
                                        <div class="col-4">Modified By:</div>
                                        <div class="col-8">{{ $invoice->modifier->FirstName }} {{ $invoice->modifier->LastName }}</div>
                                    </div>
                                @endif
                            </div>
                        </div>
