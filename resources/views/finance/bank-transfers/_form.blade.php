...
<div class="row g-3">
  <div class="col-md-4">
    ...
    <select name="FromBankAccountID" id="FromBankAccountID" class="form-select" required>
      ...
      <option value="{{ $ba->AccountID }}" @selected(old('FromBankAccountID', $row->FromBankAccountID ?? '')==$ba->AccountID)>
  ...
  <div class="col-md-4">
    ...
    <select name="ToBankAccountID" id="ToBankAccountID" class="form-select" required>
      <option value="">-- select --</option>
      @foreach($bankAccounts as $ba)
      <option value="{{ $ba->AccountID }}" @selected(old('ToBankAccountID', $row->ToBankAccountID ?? '')==$ba->AccountID)>
        {{ optional($ba->bank)->BankName }} — {{ $ba->AccountNumber }}
      </option>
      @endforeach
    </select>
  ...
  <div class="col-md-4">
    ...
    <input type="date" name="DocDate" class="form-control" value="{{ old('DocDate', $row->DocDate ?? ($defaults['DocDate'] ?? now()->toDateString())) }}" required>
  </div>
</div>
...
<div class="row g-3 mt-1">
  <div class="col-md-3">
    ...
    <select name="CurrencyID" id="CurrencyID" class="form-select" required>
      ...
      <option value="{{ $c->Id }}" data-code="{{ $c->Code }}" @selected(old('CurrencyID', $row->CurrencyID ?? '')==$c->Id)>
        {{ $c->Code }} — {{ $c->Name }}
      </option>
      @endforeach
    </select>
...
<div class="d-flex gap-2">
  ...
  <a href="{{ $isEdit ? route('finance.banktransfers.show',$row->TransferID) : route('finance.banktransfers.index') }}" class="btn btn-secondary">Cancel</a>
  @if($isEdit)
    <form action="{{ route('finance.banktransfers.post',$row->TransferID) }}" method="POST" class="ms-auto">
      @csrf
      <button type="submit" class="btn btn-primary" {{ $row->Status!=='Draft' ? 'disabled' : '' }}>Post Transfer</button>
    </form>
  @endif
</div>
...
(function(){
  ...
  function calc(){ const a=parseFloat(amt.value||0), r=parseFloat(rt.value||1); if(!isNaN(a)&&!isNaN(r)) base.textContent = 'Base: ' + (a*r).toFixed(2); else base.textContent=''; }
  amt?.addEventListener('input', calc); rt?.addEventListener('input', calc); calc();
})();
</script>
