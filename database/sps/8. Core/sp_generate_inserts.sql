...
BEGIN
    ...
    BEGIN
        ...
        -- Excluding user specified columns
        ...
        BEGIN
            IF @ommit_identity = 0 -- Determining whether to include or exclude the IDENTITY column
                SET @IDN = @Column_Name;
            ELSE
                GOTO SKIP_LOOP;
        END;
        ...
        -- Determining the data type of the column and generating the VALUES part of the INSERT statement
        SET @Actual_Values = @Actual_Values  +
        CASE
            WHEN @Data_Type IN ('char','varchar','nchar','nvarchar')
                THEN 'COALESCE('''''''' + REPLACE(RTRIM(' + @Column_Name + '),'''''''','''''''''''')+'''''''',''NULL'')'
            WHEN @Data_Type IN ('datetime','smalldatetime')
                THEN 'COALESCE('''''''' + RTRIM(CONVERT(char,' + @Column_Name + ',109))+'''''''',''NULL'')'
            WHEN @Data_Type IN ('uniqueidentifier')
                THEN 'COALESCE('''''''' + REPLACE(CONVERT(char(2000),RTRIM(' + @Column_Name + ')),'''''''','''''''''''')+'''''''',''NULL'')'
            WHEN @Data_Type IN ('text','ntext')
                THEN 'COALESCE('''''''' + REPLACE(CONVERT(char(8000),' + @Column_Name + '),'''''''','''''''''''')+'''''''',''NULL'')'
    ...
    -- Determining whether to output any debug information
    IF @debug_mode =1
    BEGIN
