CREATE OR ALTER PROCEDURE [dbo].[p_ProcessWorkflowActionTest_V1]
    @Source NVARCHAR(255),
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 1. Check if any stage exists for this Source/SourceID
        IF NOT EXISTS (
            SELECT 1
            FROM t_WorkFlowStagesTest s WITH (NOLOCK)
            INNER JOIN t_WorkFlowPendingTest p WITH (NOLOCK)
                ON s.Id = p.Stage
        ...
        BEGIN
            SET @ErrorMessage = 'No valid workflow stage found for this approval.';
            RAISERROR(@ErrorMessage, 16, 1);
            RETURN;
        END

        -- 2. Get document requirement for the current user's stage
        SELECT TOP 1
            @IsDocRequired = s.IsDocRequired
        FROM t_WorkFlowStagesTest s WITH (NOLOCK)
        INNER JOIN t_WorkFlowPendingTest p WITH (NOLOCK)
            ON s.Id = p.Stage
            AND p.SourceID = @SourceID
        ...
        -- 3. If no stage found for this specific user, error
        IF @@ROWCOUNT = 0
        BEGIN
            SET @ErrorMessage = 'No pending approval found for this user in the workflow stage.';
            RAISERROR(@ErrorMessage, 16, 1);
            RETURN;
        END

        -- 4. Only validate document/signature rules if stage requires them
        IF @IsDocRequired = 1
        BEGIN
            ...
            -- Document provided without signature
            IF @DocumentId IS NOT NULL AND @SignatureID IS NULL
            BEGIN
            ...
            -- Both document and signature provided - validate them
            IF @DocumentId IS NOT NULL AND @SignatureID IS NOT NULL
            BEGIN
        ...
        -- 11. UPDATE SOURCE TABLE IF REJECTED OR FINAL APPROVAL
