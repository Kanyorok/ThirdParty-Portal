...
async function handleRequest(request: NextRequest, method: string) {
  ...
  try {
    ...
    const requestUrl = `${apiUrl}/api/v1/portal/auth/profile/tenant`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || `Failed to ${method.toLowerCase()} tenant profile` },
    ...
    console.error(`[Tenant Profile ${method}] Request failed:`, error)
