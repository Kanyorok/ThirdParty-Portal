...
class TeamController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $data = $request->validate([
            'TeamName' => ['required', 'string', 'max:250', 'min:5', Rule::unique('t_Teams', 'Name')],
            'TeamEmail' => ['required', 'string', 'email:rfc,dns', 'max:200'],
            'TeamLead' => ['nullable', 'string'],
            'TeamMembers' => ['nullable', 'array', 'max:100'],
            'TeamNotes' => ['nullable', 'max:50000']
        ]);
        ...
        if (!empty($data['TeamLead'])) {
            ...
            if (!$user instanceof User) {
                throw ValidationException::withMessages([
                    'TeamLead' => 'user selected may be invalid'
                ]);
            }
    ...
    {
        $data = $request->validate([
            'Name' => ['required', 'string', 'max:250', 'min:5', Rule::unique('t_Teams', 'Name')->whereNotIn('TeamID', [$team->TeamID])],
            'team_lead' => ['nullable'],//Rule::exists('t_Users','UserID')
            'Email' => ['required', 'string', 'email:rfc,dns', 'max:200'],
            'Notes' => ['nullable', 'max:50000']
        ]);
        ...
        if (!empty($data['team_lead'])) {
            ...
            if (!$user instanceof User) {
                throw ValidationException::withMessages([
                    'team_lead' => 'user not a member of team'
                ]);
            }
            $userID = $user->Id;
        }
