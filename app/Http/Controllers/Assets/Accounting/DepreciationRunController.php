...
class DepreciationRunController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function() use (&$run, $data) {
            ...
            $ps = Carbon::parse($data['PeriodStart']); $pe = Carbon::parse($data['PeriodEnd']);
            foreach ($abvs as $abv) {
                // eligible window
                $depStart = $abv->DepStartDate ? Carbon::parse($abv->DepStartDate) : null;
                if ($depStart && $depStart->gt($pe)) continue;
    ...
    {
        $run = DepreciationRun::findOrFail($id);
        if ($run->Status !== 'Draft') return back()->withErrors('Only Draft can be posted.');
        ...
        DB::transaction(function() use ($run) {
            ...
            $run->Status='Posted'; $run->PostedOn=now(); $run->save();
        });
