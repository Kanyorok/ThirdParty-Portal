...
class ScheduleRequest extends FormRequest
{
    ...
    public function getEnd(Carbon $start): Carbon
    {
        ...
        if ($end->lte($start)) {
            throw ValidationException::withMessages(['schedule_end' => 'should be after start.']);
        }

        $diffInMinutes = $start->diffInMinutes($end, true);

        if ($diffInMinutes < 0) {
            throw ValidationException::withMessages(['schedule_end' => 'duration should be less least 1 minute.']);
        }

        if ($diffInMinutes > 480) {//8 hours
            throw ValidationException::withMessages(['schedule_end' => 'duration can only be a maximum of 8 hours.']);
        }
    ...
    public function getClients(): Collection
    {
        ...
        if ($clients->isEmpty()) {
            throw ValidationException::withMessages(['client' => 'No clients found.']);
        }
