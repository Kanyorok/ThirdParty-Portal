...
class BoardMeetingRequest extends FormRequest
{
    ...
    {
        return [
            'BoardMeetingCommittee' => ['required', Rule::exists('t_Committees', 'CommitteeID')],
            'BoardMeetingTitle' => ['required', 'string', 'max:200'],
            'BoardMeetingStart' => ['required', 'date_format:"Y-m-d H:i"', 'before:BoardMeetingEnd'],
            'BoardMeetingEnd' => ['required', 'date_format:"Y-m-d H:i"', 'after:BoardMeetingStart'],
            'BoardMeetingLocation' => ['required', 'string'],
            'BoardMeetingAgenda' => ['required', 'string'],
            'BoardMeetingUsers' => ['required', 'array', 'min:1', 'max:30'],
        ];
    }
    ...
    public function getCommittee(): Committee
    {
        ...
        if($committee instanceof Committee){
            if ($committee->members()->count() ===0){
                throw ValidationException::withMessages([
                    'BoardMeetingCommittee' => 'committee has no members',
                ]);
            }
            return $committee;
        }
        ...
        throw ValidationException::withMessages([
            'BoardMeetingCommittee' => 'invalid committee',
        ]);
    }
