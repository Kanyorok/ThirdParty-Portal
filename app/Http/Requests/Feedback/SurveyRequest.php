...
class SurveyRequest extends FormRequest
{
    ...
    public function getEnd(Carbon $start, string $SurveyID = null): Carbon
    {
        ...
        if (Survey::query()->where('StartOn', '<=', $end)->where('EndOn', '>=', $end)
            ->where('SurveyID', '!=', $SurveyID)->exists()) {
            throw ValidationException::withMessages([
                'End' => 'There is an overlapping survey.',
            ]);
        }
    ...
    public function getStart(string $SurveyID = null): Carbon
    {
        ...
        //check if overlapping
        if (Survey::query()->where('StartOn', '<=', $start)->where('EndOn', '>=', $start)
            ->where('SurveyID', '!=', $SurveyID)->exists()) {
            throw ValidationException::withMessages([
                'Start' => 'There is an overlapping survey.',
            ]);
        }
